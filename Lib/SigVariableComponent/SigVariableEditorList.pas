...
type
  ...
  private
    ...
    procedure SetEditorWidth(const Value: integer);
  ...
  public
    ...
    procedure Clear;
    procedure ClearFromIndex( const pIndex : integer ); // pIndex is cleared!
...
begin
  ...
  Result.Left := LeftMargin - 5;
  Result.Width := self.Width - 2 * LeftMargin;
  Result.FixedEditorWidth := self.FixedEditorWidth;
...
procedure TSigVariableEditorList.ClearFromIndex(const pIndex: integer);
begin
  while fSigVariableEditors.Count > pIndex do
  begin
...
begin
  if i < fSigVariableEditors.Count then
  begin
...
procedure TSigVariableEditorList.SetEditorWidth(const Value: integer);
var
  i : integer;
begin
  fEditorWidth := Value;
  for i := 0 to fSigVariableEditors.Count - 1 do
  begin
    Editor[ i ].FixedEditorWidth := Value;
    Editor[ i ].Invalidate;
  end;
end;
