...

{
  ...
  set up the OnUndoableAction and OnRedoableAction for the top level
...
type
  ...
  tSigFileUndoAction = ( undoClearUndoList, undoValue, undoChangeMax, undoCreate, undoInsert, undoDelete, undoSelPos, undoSelLen,
                         undoChangeActiveChild, undoMoveFrom, undoMoveTo );
  ...
  protected
    ...
    fCreating : boolean;
    fChanging : boolean; // a flag used by descendants where editor feedback is a problem, e.g. tSigMemo
    fEditorInhibited: boolean;
    fEditorUpdatePending: boolean;
    ...
    function GetEditorInhibited: boolean; virtual;
    procedure SetEditorInhibited(const Value: boolean); virtual;
    procedure SetEditorUpdatePending(const Value: boolean); virtual;
  ...
  public
    ...
    function Load( pFile : tStrings; var pLine : integer; pErrors : tErrorList = nil ) : boolean; virtual;
    ...
    property EditorInhibited : boolean
             read GetEditorInhibited
             write SetEditorInhibited;
    property EditorUpdatePending : boolean
             read fEditorUpdatePending
             write SetEditorUpdatePending;
  ...
  private
    ...
    procedure SetEditorInhibited(const Value: boolean);
    procedure SetEditorUpdatePending(const Value: boolean);
  public
    procedure AfterConstruction; override;
  ...
  protected
    ...
    procedure SetEditorInhibited(const Value: boolean); override;
    procedure SetEditorUpdatePending(const Value: boolean); override;
  public
    constructor Create( pPropertyName : string; pOwner : tSigCompoundProperty  ); override;
    destructor Destroy; override;
  ...
  tOnArrayChildMove = procedure( pFrom, pTo : integer ) of object;
  ...
  private
    ...
    fMoveFrom, fMoveTo : integer; // temporaries to allow undo/redo actions
...
//    fArrayClass: tSigBasePropertyClass;
  ...
  private
    ...
    procedure fOnMaxChange( const NewVal : integer );
  ...
  public
    constructor Create( pPropertyName : string; pOwner : tSigCompoundProperty ); override;
    procedure Clear; override;
    procedure RefreshEditor( const ForceRedraw : boolean ); virtual;
...
begin
  ...
  begin
    EditorInhibited := TRUE;
    Clear;
    if assigned( Owner ) then
    begin
      EditorInhibited := Owner.EditorInhibited;
    end
    else
    begin
      EditorInhibited := FALSE;
    end;
  end;
  Creating := FALSE;
  IsDirty := FALSE;
...
function tSigBaseProperty.GetEditorInhibited: boolean;
begin
  if fEditorInhibited then
  begin
    Result := TRUE;
  end
  else if assigned( Owner ) then
  begin
    Result := Owner.EditorInhibited;
  end
  else
  begin
    Result := FALSE;
...
function tSigBaseProperty.Load(pFile: tStrings; var pLine: integer;
...
procedure tSigBaseProperty.SetEditorInhibited(const Value: boolean);
begin
  if fEditorInhibited <> Value then
  begin
    fEditorInhibited := Value;
    if assigned( Owner ) then
    begin
      Owner.EditorInhibited := Value;
    end
    else if not Value then
    begin
      EditorUpdatePending := FALSE;  // force any pending updates
    end;
  end;
end;

procedure tSigBaseProperty.SetEditorUpdatePending(const Value: boolean);
begin
  fEditorUpdatePending := Value;
end;
  ...
  case iLoadingFromDevice of
    ...
    begin
      ...
      begin
        ...
        if not (fUndoing or Creating) then
        begin
...
function tSigSimpleProperty.Load(pFile: tStrings; var pLine: integer;
  pErrors: tErrorList): boolean;
begin
  Result := TRUE;
  IsDirty := FALSE;
...
{
function tSigPropertyList.CreateChild(const pPropertyText: string;
...
begin
  if assigned( fOwner ) then
  begin
    Result := fOwner.CreateChild( pPropertyText, pIndex, pValue, pComment, pErrors, pErrorLine, pErrorPos );
  ...
  begin
    ...
    Result := tSigCompoundProperty.Create( iText2, pIndex, fOwner);
  end
  else
  begin
    ...
    begin
      iText2 := Trim(Copy( pPropertyText, Length( iText ), Length( pPropertyText)));
      Result := tMemoProperty.Create( iText2, pIndex, fOwner);
    end
    else
    begin
      ...
      begin
        iText2 := Trim(Copy( pPropertyText, Length( iText ), Length( pPropertyText)));
        Result := tSigArrayProperty.Create( iText2, pIndex, fOwner);
      end
      else
      begin
        ...
        begin
          iText2 := Trim(Copy( pPropertyText, Length( iText ), Length( pPropertyText)));
          Result := tSigFileProperty.Create( iText2, pIndex, fOwner);
        end
        else
        begin
          Result := tSigSimpleProperty.Create( pPropertyText, pIndex, fOwner );
        end;
      end;
    end;
  end;
...
function tSigPropertyList.Load(pFile: tStrings; var pLine: integer;
   pErrors: tErrorList): boolean;
var
  SigProperty : string;
  SigIndex : string;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        iChild.Load( pFile, pLine, pErrors );
      end;
    end
    else
    begin
...
begin
  ...
  begin
    for i := 0 to Count - 1 do
    begin
      Item[ i ].IsDirty := FALSE;
    end;
  end;
end;

procedure tSigPropertyList.SetEditorInhibited(const Value: boolean);
var
  i: Integer;
begin
  fEditorInhibited := Value;
  for i := 0 to Count - 1 do
  begin
    Item[ i ].EditorInhibited := Value;
  end;
end;

procedure tSigPropertyList.SetEditorUpdatePending(const Value: boolean);
var
  i: Integer;
begin
  fEditorUpdatePending := Value;
  for i := 0 to Count - 1 do
  begin
    Item[ i ].EditorUpdatePending := Value;
  end;
end;
...
function tSigCompoundProperty.Load(pFile: tStrings; var pLine: integer;
  pErrors: tErrorList): boolean;
var
  SigProperty : string;
  SigIndex : string;
...
procedure tSigCompoundProperty.SetEditorInhibited(const Value: boolean);
begin
  if fEditorInhibited <> Value then
  begin
    inherited;
    Children.EditorInhibited := Value;
  end;
end;

procedure tSigCompoundProperty.SetEditorUpdatePending(const Value: boolean);
begin
  if Value = FALSE then
  begin
    Children.EditorUpdatePending := Value;
  end;
  inherited;
end;
...
function tMemoProperty.Load(pFile: tStrings; var pLine: integer;
  pErrors: tErrorList): boolean;
var
  SigProperty : string;
  SigIndex : string;
...
function tSigArrayProperty.Load(pFile: tStrings; var pLine: integer;
  pErrors: tErrorList): boolean;
var
  SigProperty : string;
  SigIndex : string;
...
procedure tSigArrayProperty.Move(const pFrom, pTo: integer);
var
  iSave : tObject;
  i : integer;
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      ShortCut := TextToShortcut( 'Alt+F2' );
    end;
    ...
    with fFileLoad do
    begin
      Caption := TranslateValue( '&Load' );
      OnClick := fFileLoadClick;
      ShortCut := TextToShortcut( 'Alt+F3' );
    end;
...
begin
  ...
  begin
    ...
    if not inherited Load( fSource, pLine, ErrorList ) then
    begin
      pOK := FALSE;
    end;
...
{
function tSigIntegerArray.AddNewChild: tSigBaseProperty;
begin
  Result := tSigIntegerProperty.Create( 'Item', IntToStr( Count ), self);
end;
}
...
procedure tSigBooleanProperty.SetEditorUpdatePending(const Value: boolean);
begin
  if assigned( fEdit ) then
  begin
...
procedure tSigIntegerProperty.ExecuteOnChange(
  const pChangedObject: tSigBaseProperty);
begin
  inherited;
  if assigned( fSpinEdit ) then
...
procedure tSigIntegerProperty.SetEditorUpdatePending(const Value: boolean);
begin
  if EditorUpdatePending and not Value  then
  begin
    if Assigned( fSpinEdit ) then
...
procedure tSigIntegerProperty.SetEnabled(const Value: boolean);
begin
  inherited;
  if assigned( Editor ) then
  begin
    Editor.Enabled := Value;
    Editor.EditorEnabled := Value;
    Editor.Button.Visible := Value;
    if Value then
    begin
    ...
    begin
      Editor.ParentColor := TRUE;
    end;
    Editor.MinValue := MinVal;
    Editor.MaxValue := MaxVal;
  end;
end;
...
procedure tSigIntegerProperty.SetValueAsInt(const pValue: integer);
begin
  inherited;
  if Assigned( fSpinEdit ) then
  begin
    if fSpinEdit.Value <> pValue then
    begin
      fSpinEdit.Value := pValue
    end;
  end;
end;

procedure tSigIntegerProperty.SetVisible(const Value: boolean);
begin
  fVisible := Value;
  if assigned( Editor ) then
  begin
...
procedure tSigDateTimeProperty.ExecuteOnChange(
  const pChangedObject: tSigBaseProperty);
begin
  inherited;
  if EditorInhibited then
...
procedure tSigDateTimeProperty.SetEditorUpdatePending(const Value: boolean);
begin
  if EditorUpdatePending and not Value  then
  begin
    if assigned( fEditor ) then
...
{
function tSigBooleanArray.AddNewChild: tSigBaseProperty;
begin
  Result := tSigBooleanProperty.Create( 'Item', IntToStr( Count ), self);
end;
}
...
begin
  ...
  begin
    ActiveChild := ActiveChild - 1;
  end;
end;

procedure tSigObjectList.ExecuteOnChange(
  const pChangedObject: tSigBaseProperty);
begin
  inherited;
end;
...
function tSigObjectList.Load(pFile: tStrings; var pLine: integer;
  pErrors: tErrorList): boolean;
var
  SigProperty : string;
  SigIndex : string;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        inc( PLine );
        fChildArray.Load( pFile, pLine, pErrors );
      end
      else
      begin
        break;
...
procedure tSigObjectList.SetEditorUpdatePending(const Value: boolean);
begin
  inherited;
...
begin
  inherited;
  // just provide a path for vistual creation of ascendants
end;
...
function tSigObjectArray.Load(pFile: tStrings; var pLine: integer;
  pErrors: tErrorList): boolean;
var
  SigProperty : string;
  SigIndex : string;
...
procedure tSigObjectArray.Move(const pFrom, pTo: integer);
var
  iSave : tObject;
  i : integer;
begin
...
procedure tSigObjectArray.SetEditorUpdatePending(const Value: boolean);
begin
  if EditorUpdatePending and not Value  then
  begin
    if assigned( fSpinEdit ) then
...
begin
  if fMax <> Value then
  begin
    if Value >= 0 then // can't add a negative number of children
    begin
      while Count <= Value do
      begin
        AddNewChild;
  ...
  end;
{
  ...
  if Assigned( fCountSpinEdit ) then
  begin
    if fCountSpinEdit.Value <> Value + 1 then
    begin
      fCountSpinEdit.Value := Value + 1;
    end;
  end;
  if assigned( fOnMaxChange ) then
  begin
...
procedure tSigTextProperty.ExecuteOnChange(
  const pChangedObject: tSigBaseProperty);
begin
  inherited;
  if assigned( fEdit ) then
...
procedure tSigTextProperty.SetEditorUpdatePending(const Value: boolean);
begin
  if EditorUpdatePending and not Value  then
  begin
    if assigned( fEdit ) then
...
begin
  ...
  begin
    ...
    begin
      if Visible[ i ] then
      begin
        fComboBox.Items.Add( ValueToText( i ) );
      end;
    end;
...
procedure tSigEnumProperty.SetEditorUpdatePending(const Value: boolean);
begin
  if EditorUpdatePending and not Value  then
  begin
    if assigned( fComboBox ) then
...
procedure tSigEnum<T>.ExecuteOnChange(const pChangedObject: tSigBaseProperty);
var
  iNewIndex : integer;
begin
  inherited;
  if assigned( fEdit ) then
  begin
    iNewIndex := MapValueToItem( ValueAsInt );
    if fEdit.ItemIndex <> iNewIndex then
    begin
      fEdit.ItemIndex := iNewIndex;
    end;
  end;
end;
...
procedure tSigBaseEnumProperty.SetEditorUpdatePending(const Value: boolean);
begin
  if EditorUpdatePending and not Value  then
  begin
    RefreshEditor;
...
procedure tSigStringArray.SetEditorUpdatePending(const Value: boolean);
var
  i : integer;
  iSelStart, iSelLen : integer;
begin
...
begin
  ...
  begin
    ...
    begin
      pStrings.Add(  Map[ i ].EnumName.Value );
    end;
  end;
end;
...
constructor tSigEnumMapList.Create(pPropertyName: string;
  pOwner: tSigCompoundProperty);
begin
  inherited Create( pPropertyName, pOwner, tSigEnumMap );
...
function tSigEnumMapList.Load(pFile: tStrings; var pLine: integer;
  pErrors: tErrorList): boolean;
begin
  result := inherited;
  Sort;
...
procedure tSigEnumMapList.SetActiveChild(const Value: integer);
var
...
function tSigMappedEnum.Load(pFile: tStrings; var pLine: integer;
  pErrors: tErrorList): boolean;
begin
  Result := inherited;
  RefreshEditor;
...
function tSigRelativeFileProperty.Load(pFile: tStrings; var pLine: integer;
  pErrors: tErrorList): boolean;
var
