...
type
  ...
  public
    ...
    procedure DrawCell( Canvas : TCanvas; ARow : Integer; Rect: TRect; const State: TGridDrawState; Value : string;
  ...
  public
    ...
    procedure DrawCell( Canvas : TCanvas; ACol, ARow : Integer; Rect: TRect; const State: TGridDrawState;
              Value : string; const OwnerEnabled : boolean );
...
procedure tSigEditorList.DrawCell(Canvas: TCanvas; ACol, ARow: Integer;
  Rect: TRect; const State: TGridDrawState; Value: string; const OwnerEnabled : boolean);
var
...
begin
  ...
  begin
    iEditor.DrawCell( Canvas, ARow, Rect, State, Value, OwnerEnabled );
  end
  else
  begin
...
begin
  ...
  fEditorList.DrawCell( Canvas, ACol, ARow, ARect, AState, Cell[ ACol, ARow ], Enabled );
end;
...
procedure tSigGridEditor.CheckColWidth(const Value: string);
var
  iWidth : integer;
  iAutoSizeCol : boolean;
begin
  try
    if not ParentColWidth then
    begin
      SigGrid.ColWidths[ fColumn ] := ColWidth;
      exit;
    end;
    // else
    if ParentAutoSizeColumn then
    ...
    begin
      ...
      if iWidth > SigGrid.ColWidths[ fColumn ] then
      begin
        SigGrid.ColWidths[ fColumn ] := iWidth;
...
procedure tSigGridEditor.DrawCell(Canvas: TCanvas; ARow: Integer; Rect: TRect;
  const State: TGridDrawState; Value: string; const OwnerEnabled : boolean);
var
  iImageIndex : integer;
  iValue : string;
begin
  ...
  begin
                ...
                fEditor.Width := Rect.Right - Rect.Left;
    {
          ...
          begin
            if (gdSelected in State) and OwnerEnabled then
            begin
              // resize and show editor
              if fCellBeingEdited <> ARow then
              begin
                ...
                fEditor.Width := Rect.Right - Rect.Left;
    {
            ...
            begin
              ...
              if (CellBeingEdited = ARow) or (not OwnerEnabled) then
              begin
        ...
        begin
          ...
          begin
            ...
            begin
              iImageIndex := OnGetImageIndex( Column, CellBeingEdited, State, iValue );
            end
            else if iValue = '' then
            begin
              iImageIndex := 0;
            ...
            begin
              ...
              if AutoSizeCol then
              begin
                fSigGrid.ColWidths[ fColumn ] := fImages.Width;
              end;
            end
            else
            begin
...
begin
  if fSigGrid.Cell[ Column, CellBeingEdited ] <> ComboBox.Text then
  begin
    fSigGrid.Cell[ Column, CellBeingEdited ] := ComboBox.Text;
    if assigned( fSigGrid.OnCellEditChange ) then
    begin
      fSigGrid.OnCellEditChange( Column, CellBeingEdited, ComboBox.Text );
    end;
...
begin
  if fSigGrid.Cell[ Column, CellBeingEdited ] <> MaskEdit.Text then
  begin
    fSigGrid.Cell[ Column, CellBeingEdited ] := MaskEdit.Text;
    if assigned( fSigGrid.OnCellEditChange ) then
    begin
      fSigGrid.OnCellEditChange( Column, CellBeingEdited, MaskEdit.Text );
    end;
    CheckColWidth( MaskEdit.Text );
  end;
end;
...
begin
  ...
  begin
    if fSigGrid.Cell[ Column, CellBeingEdited ] <> SpinEdit.Text then
    begin
      fSigGrid.Cell[ Column, CellBeingEdited ] := SpinEdit.Text;
      if assigned( fSigGrid.OnCellEditChange ) then
      begin
        fSigGrid.OnCellEditChange( Column, CellBeingEdited, SpinEdit.Text );
      end;
...
begin
  ...
  begin
    ...
    begin
      if Column >= 0 then
      begin
        fSigGrid.ColWidths[ Column ] := fSigGrid.DefaultColWidth;
        fSigGrid.InvalidateCol( Column );
      end;
    end;
  end
