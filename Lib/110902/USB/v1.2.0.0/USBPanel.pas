...
type
  ...
  protected
    ...
    procedure fSetOpenImage( NewVal : tImage );
    procedure fSetSendBufferImage( NewVal : tImage );
    procedure fSetRcvBufferImage( NewVal : tImage );
  ...
  published
    ...
    property OpenImage : tImage
             read iOpenImage
             write fSetOpenImage;

    property SendBufferImage : tImage
             read iSendBufferImage
             write fSetSendBufferImage;

    property RcvBufferImage : tImage
...
procedure TUSBPanel.fSetOpenImage( NewVal : tImage );
begin
  iOpenImage := NewVal;
  fShowOpenImage;
end;

procedure TUSBPanel.fSetSendBufferImage( NewVal : tImage );
begin
  iSendBufferImage := NewVal;
  fSetImage( iSendBufferImage, 4, 2 ); // empty Xmit buffer
end;

procedure TUSBPanel.fSetRcvBufferImage( NewVal : tImage );
begin
...
//    FT_Error_Check( 'writing to USB port', FT_Write( iFT_Handle, @iWriteBuffer, Length( NewVal ), @rPtr ));
  ...
  fSetImage( iSendBufferImage, 4, 3 );
  iTxStretch := 3;
end;
...
begin
  ...
  begin
    ...
    begin
      fSetImage( iSendBufferImage, 4, 2 );
    end
    else
    begin
      Dec( iTxStretch );
