...
implementation
 ...
 begin
   ...
   pcStr := @cStr;
   StringToPChar( pcStr, Str );
   Result := DrawText( DC, pcStr, -1, Rect, Format );
 end;
...
begin
	...
	iMatchLen := 1;
	for i := 1 to ResourceStream.size do begin
		ResourceStream.Read( NextChar, 2 );
		if Nextchar = FindValue[ iMatchLen ] then begin
			if iMatchLen = Length( FindValue ) then begin
				vString := '';
				ResourceStream.Read( NextChar, 2 );
				while NextChar = WideChar(0) do
					ResourceStream.Read( NextChar, 2 );
				while NextChar <> WideChar(0) do begin
