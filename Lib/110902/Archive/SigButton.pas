...
function TSigButton.SetPropertyAsInt( const PropertyID : shortstring;
                          const PropertyValue : integer ) : boolean;
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        SigButton32_MakeButtonActive( iObject );
        ActiveButton := nil; // local instance
      end
      else
      begin
        // Set ActiveButton in DLL to nil
        SigButton32_MakeButtonActive( nil );
...
//    Result := TDSMButton( iObject ).PropertyCount
    if UsesDLL then
      Result := SigButton32_GetPropertyCount( iObject )
    else
...
//    Result := TDSMButton( iObject ).PropertyName[ iIndex ]
    if UsesDLL then
      Result := SigButton32_GetPropertyName( iObject, iIndex )
    else
...
//    Result := TDSMButton( iObject ).PropertyValue[ iIndex ]
    if UsesDLL then
      Result := SigButton32_GetPropertyValue( iObject, iIndex )
    else
...
//    TDSMButton( iObject ).SetProperties;
    if UsesDLL then
    begin
      SigButton32_SetProperties( iObject );
    end
    else
    begin
...
begin
  if assigned( iObject ) then
    if usesDLL then
    begin
      SigButton32_PaintButton( iObject, iDims );
    end
    else
    begin
