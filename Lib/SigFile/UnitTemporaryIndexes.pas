...

{
  ...
  are created based on index files rather than data files.
...
type
  ...
  protected
  public
    constructor Create( const pOwner : TMemoryIndex< TIndexFile >; const pRecNo : TSigDBRecPointer; const pNodeText : string );
...
begin
  ...
  try
    while iRec <> 0 do
    begin
      fIndexFile.ReadData( iRec );
      Root.Add( TMemoryIndexNode<TIndexFile>.Create(Self, iRec, fIndexFile.NodeText ) );
...
constructor TMemoryIndexNode<TIndexFile>.Create(
  const pOwner: TMemoryIndex<TIndexFile>; const pRecNo: TSigDBRecPointer; const pNodeText : string);
begin
  inherited Create;
...
function TMemoryIndexNode<TIndexFile>.NodeText: string;
begin
  Result := fNodeText;
end;
