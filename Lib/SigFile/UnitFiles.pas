...
type
  ...
  private
    ...
    fCheckBoxAddDataIntercept: TCheckBox;
    fDeleteFieldButton: TSpeedButton;
    fLASTDirName: TSigTextProperty;
    fBLOBDirName: TSigTextProperty;
    fDBBLOBDirEditor: TEdit;
    fDBLASTDirEditor: TEdit;
    procedure SetBaseFileNameEditor(const Value: TEdit);
    procedure SetFileAdditionalTypesEditor(const Value: TMemo);
    procedure SetFieldsEditor(const Value: TSigGeneralGrid);
    procedure SetAddFieldButton(const Value: TSpeedButton);
    ...
    procedure SetCheckBoxAddIndexIntercept(const Value: TCheckBox);
    procedure SetDeleteFieldButton(const Value: TSpeedButton);
    procedure SetDBBLOBDirEditor(const Value: TEdit);
    procedure SetDBLASTDirEditor(const Value: TEdit);
  public
    constructor Create( pPropertyName : string; pOwner : tSigCompoundProperty ); override;
    ...
    property SpeedButtonAddFindFunction : TSpeedButton
             read fSpeedButtonAddFindFunction
             write SetSpeedButtonAddFindFunction;
    property DBBLOBDirEditor : TEdit
             read fDBBLOBDirEditor
             write SetDBBLOBDirEditor;
    property DBLASTDirEditor : TEdit
             read fDBLASTDirEditor
             write SetDBLASTDirEditor;
  end;
  ...
  private
    ...
    fCheckBoxAddDataIntercept: TCheckBox;
    fDeleteFieldButton: TSpeedButton;
    fBLOBDirEditor: TEdit;
    fLASTDirEditor: TEdit;
    fLASTDirName: TSigTextProperty;
    fBLOBDirName: TSigTextProperty;
    procedure SetBaseFileNameEditor(const Value: TEdit);
    ...
    procedure SetCheckBoxAddIndexIntercept(const Value: TCheckBox);
    procedure SetDeleteFieldButton(const Value: TSpeedButton);
    procedure SetBLOBDirEditor(const Value: TEdit);
    procedure SetLASTDirEditor(const Value: TEdit);
  public
    constructor Create( pPropertyName : string; pOwner : tSigCompoundProperty  ); override;
    procedure Clear; override;
    procedure RefreshEditors;
    ...
    property AdditionTypes : tMemoProperty
             read fAdditionalTypes;
    property BLOBDirName : TSigTextProperty
             read fBLOBDirName;
    property LASTDirName : TSigTextProperty
             read fLASTDirName;
    ...
    property BLOBDirEditor : TEdit
             read fBLOBDirEditor
             write SetBLOBDirEditor;
    property LASTDirEditor : TEdit
             read fLASTDirEditor
             write SetLASTDirEditor;
    const
      cTabDatabase = 0;
...
begin
  ...
  begin
    ...
    Add( '' );
    Add( 'initialization');
    Add( '  DB := TDB.Create( ExtractFilePath( Application.ExeName ) + ''DataBase'' );');
    Add( '');
    Add( 'finalization');
    Add( '  DB.Free;');
    Add( '');
...
begin
  ...
  fLASTDirName := TSigTextProperty.Create( 'LAST Directory', self );
  fBLOBDirName := TSigTextProperty.Create( 'BLOB Directory', self );
...
procedure TSigDBBFile.SetBLOBDirEditor(const Value: TEdit);
begin
  fBLOBDirEditor := Value;
  Database.DBBLOBDirEditor := Value;
end;
...
procedure TSigDBBFile.SetLASTDirEditor(const Value: TEdit);
begin
  fLASTDirEditor := Value;
  Database.DBLASTDirEditor := Value;
end;
...
begin
  ...
  begin
    ...
    case self.fCompareStyle.ValueAsStyle of
      ...
      cs_DateTime:       iLine := iLine + 'Iff(' + fFieldName.Value +'=0,'''', DateTimeToStr( ' + fFieldName.Value + '));';
...
begin
  ...
  begin
    ...
    case self.fCompareStyle.ValueAsStyle of
      ...
      cs_Text:           iLine := iLine + 'ShortString( Value );';
      cs_String:         iLine := iLine + 'ShortString( Value );';
      cs_DateTime:       iLine := iLine + 'Iff( Trim(Value) = '''', 0, StrToDateTime( Value ));';
      cs_Set_Membership: exit;
      else               exit;
    end;
    Add( iLine );
...
procedure TSigDBBDatabase.SetDBBLOBDirEditor(const Value: TEdit);
begin
  fDBBLOBDirEditor := Value;
  fBLOBDirName.Editor := Value;
end;

procedure TSigDBBDatabase.SetDBLASTDirEditor(const Value: TEdit);
begin
  fDBLASTDirEditor := Value;
end;
...
begin
  ...
  begin
    ...
    if FindLevel.ValueAsInt > 0 then
    begin
      case self.FindMode.ValueAsEnum of
        ff_Variable,
        ff_First,
    ...
    if FindLevel.ValueAsInt = 0 then
    begin
      iLine := iLine + ' ) : tSigDBRecPointer; reintroduce; overload;';
    end
    else
    begin
      iLine := iLine + IndexFile.KeyParm( FindLevel.ValueAsInt - 1 ) + ' ) : tSigDBRecPointer; reintroduce; overload;';
    end;
    Add( iLine );
    Add( '' );
...
begin
  ...
  begin
    ...
    case FindMode.ValueAsEnum of
      ...
      ff_First:
      begin
        if iFindLevel = 0 then
        begin
          Add( 'function T' + IndexFile.FullName + '.First( var pCurr: tSigDBIndex ) : TSigDBRecPointer;');
        end
        else
        begin
      ...
      begin
        if iFindLevel = 0 then
        begin
          Add( 'function T' + IndexFile.FullName + '.Next( var pCurr: tSigDBIndex ) : TSigDBRecPointer;');
        end
        else
        begin
      ...
      begin
        if iFindLevel = 0 then
        begin
          Add( 'function T' + IndexFile.FullName + '.Prev( var pCurr: tSigDBIndex ) : TSigDBRecPointer;');
        end
        else
        begin
      ...
      begin
        if iFindLevel = 0 then
        begin
          Add( 'function T' + IndexFile.FullName + '.Last( var pCurr: tSigDBIndex ) : TSigDBRecPointer;');
        end
        else
        begin
