...
type
  ...
  TSigDBIndexFieldList = class( TTypedObjectList<TSigDBIndexField> )
  private
  ...
  protected
    ...
    function GetLastInserted: Int64; virtual;
    function GetLastDeleted: Int64; virtual;
  ...
  public
    ...
    property RecCount : Int64
             read GetRecCount;
    property LastInserted : Int64
             read GetLastInserted;
    property LastDeleted : Int64
             read GetLastDeleted;
  ...
  TInterestedPartiesList = class( TTypedObjectList< TInterestedParty>)
  private
    { Use a descendant of this to act whenever database file is modified and register it
      The functions in this do nothing. Override the functions needed. }
  protected
  ...
  protected
    ...
    function GetLastInserted: Int64; override;
    function GetLastDeleted: Int64; override;
  ...
  TSigDBFileBaselist = class( TTypedObjectList< TSigDBFileBase > )
  private
  ...
  public
    ...
    function First( var pCurr : tSigDBIndex; const pFieldCount : integer ) : tSigDBRecPointer; overload;// finds the record in the data file with the key supplied in fFields0
    function Next( var pCurr : tSigDBIndex )  : tSigDBRecPointer; overload; virtual;
    function Next( var pCurr : tSigDBIndex; const pFieldCount : integer )  : tSigDBRecPointer; overload; virtual;

    function Last( var pCurr : tSigDBIndex ) : tSigDBRecPointer; overload; virtual; // points to DB File!
    function Last( var pCurr : tSigDBIndex; const pFieldCount : integer ) : tSigDBRecPointer; overload;// finds the record in the data file with the key supplied in fFields0
    function Prev( var pCurr : tSigDBIndex )  : tSigDBRecPointer; overload; virtual;
    function Prev( var pCurr : tSigDBIndex; const pFieldCount : integer )  : tSigDBRecPointer; overload; virtual;
...
begin
  ...
  begin
    case FieldType of
      ...
      ft_Filter_LT:
      begin
        Result := DataFile.CompareField( pIndex, pDBRec, Filter ) = crLT;
      end;
      ft_Filter_LE:
      begin
        Result := DataFile.CompareField( pIndex, pDBRec, Filter ) in [crLT, crEQ];
      end;
      ft_Filter_EQ:
      begin
        Result := DataFile.CompareField( pIndex, pDBRec, Filter ) = crEQ;
      end;
      ft_Filter_NE:
      begin
        Result := DataFile.CompareField( pIndex, pDBRec, Filter ) in [crLT, crGT];
      end;
      ft_Filter_GE:
      begin
        Result := DataFile.CompareField( pIndex, pDBRec, Filter ) in [crEQ, crGT];
      end;
      ft_Filter_GT:
      begin
        Result := DataFile.CompareField( pIndex, pDBRec, Filter ) = crGT;
      end;
      ft_Filter_in:
      begin
  ...
  begin
    ...
    case fFields.Compare( pCurr.SigDBIndex, pFieldCount ) of
      ...
      begin
        ...
        begin
          iResult := First( iCurr, pFieldCount );
          if iResult <> 0 then
          begin
            pCurr := iCurr;
            Result := iResult;
  ...
  begin
    ...
    case fFields.Compare( pCurr.SigDBIndex, pFieldCount ) of
      ...
      begin
        ...
        if assigned( iCurr ) then
        begin
          iResult := Last( iCurr, pFieldCount );
          if iResult <> 0 then
          begin
            pCurr := iCurr;
            Result := iResult;
...
procedure tSigDBDataFile<T>.AfterRecWrite;
begin
  inherited;
...
begin
  // remove associated record in index file
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        fRootRec.Rec.InsLast := fFields0.Rec.Next;
        // don't write yet - it will be written later in this procedure.
      end;
      if fFields0.Rec.Prev = 0 then
      begin
        fFields1RecNo := 0;
      end
      else
...
function tSigDBFileBase<T>.GetLastDeleted: Int64;
begin
  if Open then
  begin
    Result := fRootRec.Rec.DelLast;
...
function tSigDBFileBase<T>.GetLastInserted: Int64;
begin
  if Open then
  begin
    Result := fRootRec.Rec.InsLast;
...
function tSigDBFileBase.GetLastDeleted: Int64;
begin
  raise exception.Create( 'Invalid LAST DELETED request' );
end;

function tSigDBFileBase.GetLastInserted: Int64;
begin
...
begin
  ...
  begin
    try
      Item[ i ].AfterRecAppend( Sender, pRec );
    except
      // don't abort on error - is clients responsibility totrap errors and act on them
    end;
  end;
...
begin
  ...
  begin
    try
      Item[ i ].AfterRecordDelete( Sender );
    except
      // don't abort on error - is clients responsibility totrap errors and act on them
    end;
  end;
...
begin
  ...
  begin
    try
      Item[ i ].AfterRecRead( Sender, pRec );
    except
      // don't abort on error - is clients responsibility totrap errors and act on them
    end;
  end;
...
begin
  ...
  begin
    try
      Item[ i ].AfterRecWrite( Sender, pRec );
    except
      // don't abort on error - is clients responsibility totrap errors and act on them
    end;
  end;
...
begin
  ...
  begin
    try
      Item[ i ].BeforeRecordDelete( Sender, pRec );
    except
      // don't abort on error - is clients responsibility totrap errors and act on them
    end;
  end;
...
begin
  ...
  begin
    try
      Item[ i ].BeforeRecWrite( Sender, pRec );
    except
      // don't abort on error - is clients responsibility totrap errors and act on them
    end;
  end;
...
begin
  ...
  begin
    try
      if not Item[ i ].CanDelete( Sender, pRec ) then
      begin
        Result := FALSE;
        exit;
      end;
