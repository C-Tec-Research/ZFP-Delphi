...
type
  ...
  private
    ...
    fUsesFirstNext: TSigBooleanProperty;
    fFirstLevel: tSigIntegerProperty;
    function GetDataFile: tSigDBBBaseFile;
  ...
  private
    ...
    fSpeedButtonAddIndexFile: TSpeedButton;
    fEditIndexExtension: TEdit;
    fSigGeneralGridIndexes: TSigGeneralGrid;
    fSigSpinEditIndexCount: TSigSpinEdit;
    fSigGeneralGridFind: tSigGeneralGrid;
    ...
    procedure SetCheckBoxMirrorDataFields(const Value: TCheckBox);
    procedure SetCheckBoxAddIndexIntercept(const Value: TCheckBox);
    procedure SetSpeedButtonDeleteIndexFile(const Value: TSpeedButton);
  protected
    procedure ShowTabIndexFiles;
    procedure OnTabIndexChange( Sender : tObject );
    procedure SetActiveChild(const Value: integer); override;
    procedure OnSpeedButtonAddIndexFileClick( Sender : TObject );
    procedure OnSpeedButtonDeleteIndexFileClick( Sender : TObject );
  public
    constructor Create( pPropertyName : string; pOwner : tSigCompoundProperty ); override;
    ...
    property SpeedButtonAddIndexFile : TSpeedButton
             read fSpeedButtonAddIndexFile
             write SetSpeedButtonAddIndexFile;
    property EditIndexExtension : TEdit
             read fEditIndexExtension
             write SetEditIndexExtension;
    property CheckBoxMirrorDataFields : TCheckBox
    ...
    property SpeedButtonAddFindFunction : TSpeedButton
             read fSpeedButtonAddFindFunction
             write SetSpeedButtonAddFindFunction;
    property SpeedButtonDeleteIndexFile : TSpeedButton
             read fSpeedButtonDeleteIndexFile
             write SetSpeedButtonDeleteIndexFile;
  end;
  ...
  private
    fFilesListEditor: TTabControl;
    fAddFileButton: TSpeedButton;
    fBaseFileNameEditor: TEdit;
    fFileAdditionalTypesEditor: TMemo;
    fFieldsEditor: TSigGeneralGrid;
    fAddFieldButton: TSpeedButton;
    ...
    fDeleteFieldButton: TSpeedButton;
    fSpeedButtonDeleteIndexFile: TSpeedButton;
    procedure SetFilesListEditor(const Value: TTabControl);
    function GetBaseFile(const i: integer): tSigDBBBaseFile;
    procedure OnTabChange( sender : tObject );
    procedure OnAddFileClick( sender : TObject );
    procedure SetAddFileButton(const Value: TSpeedButton);
    procedure SetBaseFileNameEditor(const Value: TEdit);
    procedure SetFileAdditionalTypesEditor(const Value: TMemo);
    procedure SetFieldsEditor(const Value: TSigGeneralGrid);
    procedure SetAddFieldButton(const Value: TSpeedButton);
  ...
  public
    ...
    property FilesListEditor : TTabControl
             read fFilesListEditor
             write SetFilesListEditor;
    property AddFileButton : TSpeedButton
             read fAddFileButton
             write SetAddFileButton;
    property BaseFileNameEditor : TEdit
             read fBaseFileNameEditor
             write SetBaseFileNameEditor;
    property FileAdditionalTypesEditor : TMemo
...
procedure tSigDBBFile.SetAddFileButton(const Value: TSpeedButton);
begin
  fAddFileButton := Value;
  DataBase.AddFileButton := Value;
end;
...
procedure tSigDBBFile.SetFilesListEditor(const Value: TTabControl);
begin
  fFilesListEditor := Value;
  DataBase.FilesListEditor := Value;
end;
...
procedure tSigDBBFile.SetSpeedButtonAddIndexFile(const Value: TSpeedButton);
begin
  fSpeedButtonAddIndexFile := Value;
  Database.SpeedButtonAddIndexFile := Value;
end;

procedure tSigDBBFile.SetSpeedButtonDeleteIndexFile(const Value: TSpeedButton);
begin
  fSpeedButtonDeleteIndexFile := Value;
  Database.SpeedButtonDeleteIndexFile := Value;
end;

procedure tSigDBBFile.SetTabControlIndexFiles(const Value: TTabControl);
begin
...
begin
  ...
  begin
    ...
    case self.fCompareStyle.ValueAsStyle of
      ...
      cs_DateTime:       iLine := iLine + 'CompareDateTime( pRec.' + fFieldName.Value + ', StrToDateTime( pFilter ));';
...
procedure tSigDBBBaseFile.SetSpeedButtonAddIndexFile(const Value: TSpeedButton);
begin
  fSpeedButtonAddIndexFile := Value;
  IndexFiles.SpeedButtonAddIndexFile := Value;
end;

procedure tSigDBBBaseFile.SetSpeedButtonDeleteIndexFile(
  const Value: TSpeedButton);
begin
  fSpeedButtonDeleteIndexFile := Value;
  IndexFiles.SpeedButtonDeleteIndexFile := Value;
end;

procedure tSigDBBBaseFile.SetTabControlIndexFiles(const Value: TTabControl);
begin
  fTabControlIndexFiles := Value;
  IndexFiles.TabControlIndexFiles := Value;
end;
...
function tSigDBBaseFileList.Load(pFile: tStrings; var pLine: integer;
  const pAllowUndo, pIsDirty: boolean; pErrors: tErrorList): boolean;
begin
  Result := inherited;
end;

procedure tSigDBBaseFileList.OnAddFileClick(sender: TObject);
begin
  Max := Max + 1;
end;

procedure tSigDBBaseFileList.OnTabChange(sender: tObject);
begin
  if ActiveChild <> fFilesListEditor.TabIndex then
  begin
    SetActiveChild( fFilesListEditor.TabIndex );
  end;
end;
...
procedure tSigDBBaseFileList.RemoveEditors;
begin
  inherited;
...
begin
  ...
  begin
    ...
    begin
      ...
      TabControlIndexFiles := nil;
      SpeedButtonAddIndexFile := nil;
      ...
      SpeedButtonDeleteIndexFile := nil;
    end;
  end;
  inherited;
  if assigned( fFilesListEditor ) then
  begin
    if fFilesListEditor.TabIndex <> pValue then
    begin
      fFilesListEditor.TabIndex := pValue;
    end;
    ...
    begin
      ...
      begin
        ...
        TabControlIndexFiles := self.TabControlIndexFiles; // self required here
        SpeedButtonAddIndexFile := self.SpeedButtonAddIndexFile; // self required here
        ...
        SpeedButtonDeleteIndexFile := self.SpeedButtonDeleteIndexFile;
      end;
    end
    else
    begin
...
procedure tSigDBBaseFileList.SetAddFileButton(const Value: TSpeedButton);
begin
  if assigned( fAddFileButton ) then
  begin
    fAddFileButton.OnClick := nil;
    fAddFileButton.Enabled := FALSE;
  end;
  fAddFileButton := Value;
  if assigned( fAddFileButton ) then
  begin
    fAddFileButton.OnClick := OnAddFileClick;
    fAddFileButton.Enabled := TRUE;
  end;
end;
...
procedure tSigDBBaseFileList.SetFilesListEditor(const Value: TTabControl);
begin
  if assigned( fFilesListEditor ) then
  begin
    fFilesListEditor.OnChange := nil;
  end;
  fFilesListEditor := Value;
  if assigned( fFilesListEditor ) then
  begin
    fFilesListEditor.OnChange := OnTabChange;
  end;
  ShowTabs;
end;

procedure tSigDBBaseFileList.SetMax(const Value: integer);
begin
  inherited;
  ShowTabs;
end;
...
procedure tSigDBBaseFileList.SetSpeedButtonAddIndexFile(
  const Value: TSpeedButton);
begin
  fSpeedButtonAddIndexFile := Value;
  if ActiveChild >= 0 then
  begin
    BaseFile[ ActiveChild ].SpeedButtonAddIndexFile := Value;
  end;
end;

procedure tSigDBBaseFileList.SetSpeedButtonDeleteIndexFile(
  const Value: TSpeedButton);
begin
  fSpeedButtonDeleteIndexFile := Value;
  if ActiveChild>= 0 then
  begin
    BaseFile[ ActiveChild ].SpeedButtonDeleteIndexFile := Value;
  end;
end;

procedure tSigDBBaseFileList.SetTabControlIndexFiles(const Value: TTabControl);
begin
  fTabControlIndexFiles := Value;
  if ActiveChild >= 0 then
  begin
    BaseFile[ ActiveChild ].TabControlIndexFiles := Value;
  end;
end;

procedure tSigDBBaseFileList.ShowTabs;
var
  i: integer;
begin
  if assigned( fFilesListEditor ) then
  begin
    fFilesListEditor.Visible := Max >= 0;
    with fFilesListEditor.Tabs do
    begin
      Clear;
      for i := 0 to Max do
      begin
        Add( BaseFile[ i ].BaseFileName.Value );
      end;
    end;
    fFilesListEditor.TabIndex := ActiveChild;
  end;
end;
...
procedure tSigDBBDatabase.SetAddFileButton(const Value: TSpeedButton);
begin
  fAddFileButton := Value;
  Files.AddFileButton := Value;
end;
...
procedure tSigDBBDatabase.SetFilesListEditor(const Value: TTabControl);
begin
  fFilesListEditor := Value;
  Files.FilesListEditor := Value;
end;
...
procedure tSigDBBDatabase.SetSpeedButtonAddIndexFile(const Value: TSpeedButton);
begin
  fSpeedButtonAddIndexFile := Value;
  Files.SpeedButtonAddIndexFile := Value;
end;

procedure tSigDBBDatabase.SetSpeedButtonDeleteIndexFile(
  const Value: TSpeedButton);
begin
  fSpeedButtonDeleteIndexFile := Value;
  Files.SpeedButtonDeleteIndexFile := Value;
end;

procedure tSigDBBDatabase.SetTabControlIndexFiles(const Value: TTabControl);
begin
  fTabControlIndexFiles := Value;
  Files.TabControlIndexFiles := Value;
end;
...
begin
  Result := DataFile.BaseFileName.Value + 'Index' + NameExtension.Value;
end;
...
procedure tSigDBBIndexFileList.OnSpeedButtonAddIndexFileClick(Sender: TObject);
begin
  Max := Max + 1;
  ShowTabIndexFiles;
end;

procedure tSigDBBIndexFileList.OnSpeedButtonDeleteIndexFileClick(
  Sender: TObject);
begin
  if ActiveChild > 0 then
  begin
...
procedure tSigDBBIndexFileList.OnTabIndexChange(Sender: tObject);
begin
  if ActiveChild <> fTabControlIndexFiles.TabIndex then
  begin
    ActiveChild := fTabControlIndexFiles.TabIndex;
  end;
end;
...
procedure tSigDBBIndexFileList.RemoveEditors;
begin
  inherited;
...
begin
  ...
  if assigned( fTabControlIndexFiles ) then
  begin
    if ActiveChild <> fTabControlIndexFiles.TabIndex then
    begin
      fTabControlIndexFiles.TabIndex := ActiveChild;
    end;
  end;
end;
...
procedure tSigDBBIndexFileList.SetSpeedButtonAddIndexFile(
  const Value: TSpeedButton);
begin
  if assigned( fSpeedButtonAddIndexFile ) then
  begin
    fSpeedButtonAddIndexFile.OnClick := nil;
  end;
  fSpeedButtonAddIndexFile := Value;
  if assigned( fSpeedButtonAddIndexFile ) then
  begin
    fSpeedButtonAddIndexFile.OnClick := OnSpeedButtonAddIndexFileClick;
  end;
end;

procedure tSigDBBIndexFileList.SetSpeedButtonDeleteIndexFile(
  const Value: TSpeedButton);
begin
  if assigned( fSpeedButtonDeleteIndexFile ) then
  begin
    fSpeedButtonDeleteIndexFile.OnClick := nil;
  end;
  fSpeedButtonDeleteIndexFile := Value;
  if assigned( fSpeedButtonDeleteIndexFile ) then
  begin
    fSpeedButtonDeleteIndexFile.OnClick := OnSpeedButtonDeleteIndexFileClick;
  end;
end;

procedure tSigDBBIndexFileList.SetTabControlIndexFiles(const Value: TTabControl);
begin
  if assigned( fTabControlIndexFiles ) then
  begin
    fTabControlIndexFiles.OnChange := nil;
  end;
  fTabControlIndexFiles := Value;
  if assigned( fTabControlIndexFiles ) then
  begin
    fTabControlIndexFiles.OnChange := OnTabIndexChange;
  end;
  ShowTabIndexFiles;
end;

procedure tSigDBBIndexFileList.ShowTabIndexFiles;
var
  i: integer;
begin
  if assigned( fTabControlIndexFiles ) then
  begin
    with fTabControlIndexFiles do
    begin
      if Max >= 0 then
      begin
        Visible := TRUE;
        ...
        begin
          ...
          for i := 0 to Max do
          begin
            Add( SigDBBIndexFile[ i ].NameExtension.Value );
          end;
        end;
...
begin
  ...
  begin
    ...
    if not UsesFirstNext.ValueAsBool then
    begin
      exit;
    end;

    iLine := '    function First( var pCurr: tSigDBIndex; ';
    for i := 0 to FirstLevel.ValueAsInt - 2 do
    begin
      iLine := iLine + IndexFile.KeyParm( i ) + ';';
      Add( iLine );
      iLine := '        ';
    end;
    Add( iLine + IndexFile.KeyParm( FirstLevel.ValueAsInt - 1 ) + ' ) : tSigDBRecPointer; overload;' );
    Add( '' );
    iLine := '    function Next( ';
    iLine := iLine + 'var pCurr: tSigDBIndex; ';
    for i := 0 to FirstLevel.ValueAsInt - 2 do
    begin
      iLine := iLine + IndexFile.KeyParm( i ) + ';';
      Add( iLine );
...
begin
  ...
  begin
    ...
    Add( iLine + IndexFile.KeyParm( FindLevel.ValueAsInt - 1 ) + ' ) : TSigDBRecPointer;' );
    Add('begin');
    Add('  Database.Lock;');
    Add('  try');
    Add('    with ' + DataFile.BaseFileName.Value + 'Data do');
    Add('    begin');
    for i := 0 to FindLevel.ValueAsInt - 1 do
    begin
    ...
    case FindMode.ValueAsEnum of
      ff_Variable:
      begin
        Add('      Result := Find( pMode, ' + FindLevel.Value + ' );');
      end;
      ff_EQ:
      begin
        Add('      Result := Find( fmEQ, ' + FindLevel.Value + ' );');
      end;
      ff_LE:
      begin
        Add('      Result := Find( fmLE, ' + FindLevel.Value + ' );');
      end;
      ff_GE:
      begin
        Add('      Result := Find( fmGE, ' + FindLevel.Value + ' );');
      end;
      else
      begin
    ...
    Add( iLine + IndexFile.KeyParm( FirstLevel.ValueAsInt - 1 ) + ' ) : TSigDBRecPointer;' );
    Add('begin');
    Add('  Database.Lock;');
    Add('  try');
    Add('    with ' + DataFile.BaseFileName.Value + 'Data do');
    Add('    begin');
    for i := 0 to FirstLevel.ValueAsInt - 1 do
    begin
      with IndexFile.IndexField[ i ].DBBField.DBBField do
      begin
        Add('      ' + FieldName.Value + ' := p' + FieldName.Value + ';');
    ...
    Add('    end;');
    Add('    Result := inherited First( pCurr, ' + FirstLevel.Value + ' );');
    Add('  finally');
    Add('    Database.Unlock;');
    Add('  end;');
    Add('end;');
    ...
    Add( iLine + IndexFile.KeyParm( FirstLevel.ValueAsInt - 1 ) + ' ) : TSigDBRecPointer;' );
    Add( 'begin');
    Add( '  { the parameters (except for pCurr) are not used except to make the ');
    Add( '    function signature unique and associated with the appropriate version of ''First'' }');
    Add( '  Database.Lock;');
    Add( '  try');
    Add( '    Result := inherited Next( pCurr, ' + FirstLevel.Value + ' );' );
    Add( '  finally');
    Add( '    Database.Unlock;');
    Add( '  end;');
    Add( 'end;');
    ...
    Add('    end;');
    Add('    Result := inherited Last( pCurr, ' + FirstLevel.Value + ' );');
    Add('  finally');
    Add('    Database.Unlock;');
    Add('  end;');
    Add('end;');
    ...
    Add( 'begin');
    Add( '  { the parameters (except for pCurr) are not used except to make the ');
    Add( '    function signature unique and associated with the appropriate version of ''Last'' }');
    Add( '  Database.Lock;');
    Add( '  try');
    Add( '    Result := inherited Prev( pCurr, ' + FirstLevel.Value + ' );' );
    Add( '  finally');
    Add( '    Database.Unlock;');
    Add( '  end;');
    Add( 'end;');
...
begin
  ...
  fUsesFirstNext := TSigBooleanProperty.Create( 'Create First/Next Procedures', self );
  fFirstLevel := tSigIntegerProperty.Create( 'First Level', self );
end;
