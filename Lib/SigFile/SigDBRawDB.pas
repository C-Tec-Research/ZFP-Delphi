...
type
  ...
  public
    ...
    //constructor Create( const pFrom : TSigDBIndex ); overload; virtual; // creates an exact copy of an existing index item
    {
    ...
    //procedure Assign( const pFrom : TSigDBIndex ); virtual; don't work!
  ...
  IUserData = interface
    ['{A16335E7-705A-42CC-AF6B-E07E00FE9FD0}']
    function Clone : IUserData; // clones the current object
  end;
  ...
  public
    ...
    function UserData : IUserData; virtual; abstract;
    ...
    function FieldAsText( const pFieldID : integer ) : string; virtual; // will need to add locale later
  ...
  //TOnCopyUserData = procedure( var pDest :TInterfacedObject; const pSource : TInterfacedObject ) of object;

  {
  TUserData = class( TInterfacedObject )
  public
    class function Clone( const pSrc : TUserData ) : TUserData; virtual; abstract; // creates an instance of the class
  end;
  }
  ...
  protected
    ...
    type tD2 = record
      ...
      UserData : IUserData;  // for random user data
      class operator Positive( pSrc : tD2 ) : tD2;
    end;
  protected
    //class var fOnCopyUserData :  TOnCopyUserData;
  protected
    fFile : File;
    ...
    //class procedure CopyUserData( var pDest :TInterfacedObject; const pSource : TInterfacedObject );// virtual;
  ...
  public
    ...
    function Delete( pRec : tSigDBRecPointer ) : boolean; override;
    function Append( var pBuffer : tSigDBFileBase< T >.Td2; var pIndex : TSigDBRecPointer ) : tSigDBRecPointer; override; // 0 = fail
    ...
    function UserData: IUserData; override;
  end;
  ...
  public
    ...
    function Find( pMode : tSigDBFindMode; var pCurr : TSigDBIndex; const pFieldCount : integer = -1 ) : tSigDBRecPointer; overload;// finds the record in the data file with the key supplied in fFields0
    function FindUpdateRec( var pNode : TSigDBIndex; const pRec : tSigDBRecPointer ) : tSigDBRecPointer; //overload;// finds the record in the data file with the key supplied in fFields0
    function FindDupUpdateRec(var pNode : TSigDBIndex; const pRec : tSigDBRecPointer ) : tSigDBRecPointer; //overload;// finds the record in the data file with the key supplied in fFields0
    ...
    procedure DeleteIndex( const pDataFile : tSigDBFileBase; const pRec : tSigDBRecPointer ); virtual;
    //procedure DeleteIndex( const pRec : tSigDBRecPointer ); overload;
    procedure DeleteUpdateIndex( const pDataFile : tSigDBFileBase; const pRec : tSigDBRecPointer ); virtual;
    procedure PrepareForUpdate( const pDataFile : tSigDBFileBase; const pRec : tSigDBRecPointer ); virtual;
    procedure UpdateIndex( const pDataFile : tSigDBFileBase; const pRec : tSigDBRecPointer ); virtual;
  ...
  public
    ...
    procedure PrepareForUpdate( const pDataFile : TSigDBFileBase; const pRec : tSigDBRecPointer );
  ...
  private
    ...
    fLock: TObject;
    fOpen: boolean;
    fPendingLockCount: integer;
    fCurrentLockCount: integer;
  ...
  public
    ...
    property PendingLockCount : integer
             read fPendingLockCount;
    property CurrentLockCount : integer
             read fCurrentLockCount;
    property Open : boolean
             read fOpen
             write SetOpen;
  end;
...
begin
  inc( fPendingLockCount );
  TMonitor.Enter( fLock );
  dec( fPendingLockCount );
  inc( fCurrentLockCount );
end;

procedure TSigDBDatabase.PrepareForUpdate(const pDataFile: tSigDBFileBase;
  const pRec: tSigDBRecPointer);
begin
  Lock;
  try
    fIndexFiles.PrepareForUpdate( pDataFile, pRec );
  finally
    Unlock;
  end;
end;
...
procedure TSigDBIndexFileList.PrepareForUpdate(const pDataFile: TSigDBFileBase;
  const pRec: tSigDBRecPointer);
var
  i: Integer;
begin
  for i := 0 to Count-1 do
  begin
    IndexFile[ i ].PrepareForUpdate( pDataFile, pRec );
...
destructor TSigDBIndexFieldList.Destroy;
begin
  fRoot.Free;
  inherited;
end;
...
begin
  ...
  begin
    ...
    fBTreeRootRec := +fTempRec; // not used by data files
  end;
end;
...
procedure tSigDBIndexFile.DeleteUpdateIndex(const pDataFile: tSigDBFileBase; const pRec : tSigDBRecPointer);
var
  iToDelete : tSigDBRecPointer;
  iNode : TSigDBIndex;
begin
  ...
  try
    ...
    begin
      DataFile.Lock;
      try
        iToDelete := FindUpdateRec( iNode, pRec );
        if iToDelete > 0 then
        begin
          Delete( iToDelete );
          iNode.RemoveAndFree;
...
destructor TSigDBIndexFile.Destroy;
begin
  fFields.Free;
  inherited;
end;
...
begin
  case Field[ pIndex ].FieldType of
    ...
    ft_Filter_GE, ft_Filter_GT: Result := IsSingleValue( pValue );
    ft_Filter_EQ, ft_Filter_NE,
    ft_Filter_in, ft_Filter_not_in: Result := IsValidList( pValue );
    else
    begin
      Result := FALSE;
    end;
  ...
  begin
    ...
    case fFields.Compare( pCurr.SigDBIndex, pFieldCount ) of
      ...
      begin
        ...
        begin
          Result := 0;
          if assigned( iCurrSave ) then
          begin
            pCurr := iCurrSave;
            Result := iCurrSave.SigDBIndex;
          end;
          exit;
        end
        else
...
function TSigDBIndexFile.Find(pMode: tSigDBFindMode; var pCurr: tSigDBIndex;
  const pFieldCount: integer): tSigDBRecPointer;
begin
  Result := Find( pMode, pFieldCount );
  if Result = 0 then
  begin
    pCurr := nil;
  end
...
function TSigDBIndexFile.FindDupUpdateRec(
  var pNode: TSigDBIndex; const pRec : TSigDBRecPointer): tSigDBRecPointer;
var
  iCurrent, iDup : TSigDBIndex;
begin
  iCurrent := pNode;
  if assigned( iCurrent ) then
  begin
    if iCurrent.fSigDBIndex = pRec then
    begin
      // Need to check first because of duplicate keys
      Result := iCurrent.fCurrIndex;
      pNode := iCurrent;
      exit;
    end
    else
    begin
      case fFields.CompareUpdateRec( iCurrent.SigDBIndex ) of
        crEQ:
        begin
          iDup := iCurrent.LeftChild as TSigDBIndex;
          Result := FindDupUpdateRec( iDup, pRec );
          if Result = 0 then
          begin
            iDup := iCurrent.RightChild as TSigDBIndex;
            Result := FindDupUpdateRec( iDup, pRec );
            if Result > 0 then
            begin
              pNode := iDup;
...
function TSigDBIndexFile.FindUpdateRec( var pNode : TSigDBIndex; const pRec : tSigDBRecPointer): tSigDBRecPointer;
var
  iCurrent, iDup : TSigDBIndex;
begin
  if not Open then
  ...
  begin
    ...
    case fFields.CompareUpdateRec( iCurrent.SigDBIndex ) of
      crEQ:// match found
      begin
        // if we get here it is a duplicate key, because data file record numbers differ.
        // there may be more records to left and right, so we search these
        iDup := iCurrent.LeftChild as TSigDBIndex;
        Result := FindDupUpdateRec( iDup, pRec );
        if Result = 0 then
        begin
...
function tSigDBIndexFile.UserData: IUserData;
begin
...
begin
  raise Exception.Create('Filter Validation not implemented');  // must be implemented by descendant classes if filter validation is to be used
end;
...
begin
  ...
  begin
    ...
    case fFields.Compare( pCurr.SigDBIndex, pFieldCount ) of
      ...
      begin
        Result := pCurr.SigDBIndex;
        // it is possible that there are duplicate keys so make sure that we have the highest
        // we know it must be to the right of us so...
...
begin
  ...
  Lock;
  try
    if assigned( Root.LeftChild ) then
    begin
      pCurr := Root.LeftChild.Last as tSigDBIndex;
      if assigned( pCurr ) then
      begin
        Result := pCurr.fSigDBIndex;
        //DataFile.Read( Result );
...
procedure TSigDBIndexFile.PrepareForUpdate(const pDataFile: tSigDBFileBase;
  const pRec: tSigDBRecPointer);
begin
  if DataFile = pDataFile then
  begin
    ...
    begin
      ...
      DeleteUpdateIndex( pDataFile, pRec );
...
begin
  ...
  if Read( pRec, fUpdateRec, fUpdateRecNo ) then DataBase.PrepareForUpdate( self, fUpdateRecNo );
  // and restore rec0 to the temporary buffer
  //fTempRec.Rec := fFields0.Rec;
  fTempRec := +fFields0;
end;
...
function tSigDBDataFile<T>.UserData: IUserData;
begin
...
{
procedure TSigDBIndex.Assign(const pFrom: TSigDBIndex);
begin
  fLastAccessed := Now();
  fLeftChildIndex := pFrom.fLeftChildIndex;
  fRightChildIndex := pFrom.fRightChildIndex;
  fSigDBIndex := pFrom.fSigDBIndex;
  fIndexFile := pFrom.fIndexFile;
  fCurrIndex := pFrom.fCurrIndex;
  fParent := pFrom.fParent;
(*
  if assigned( fLeftChild ) then
  begin
    FreeAndNil( fLeftChild );
  end;
  if assigned( pFrom.fLeftChild ) then
  begin
    fLeftChild := TSigDBIndex.Create( pFrom.fLeftChild as TSigDBIndex );
  end;
  ...
  if assigned( fRightChild ) then
  begin
    fRightChild := TSigDBIndex.Create( pFrom.fRightChild as TSigDBIndex );
  end;
*)
  fLeftChild := nil;
  fRightChild := nil;
...
{
constructor TSigDBIndex.Create(const pFrom: TSigDBIndex);
begin
  inherited Create;
  Assign( pFrom );
...
begin
  ...
  fLeftChildIndex := Result; // for updates
end;
...
{
class procedure TSigDBFileBase<T>.CopyUserData( var pDest :TInterfacedObject; const pSource : TInterfacedObject);
begin
  // for most records does nothing. Only if pSource is defined
  // does it do anything.
  if assigned( fOnCopyUserData ) then
  begin
    fOnCopyUserData( pDest, pSource );
  end
  else if assigned( pSource ) then
  begin
    raise Exception.Create('invalid user data copy');
  end;
end;
}
...
begin
  ...
  begin
    ...
    fTempRec := +fFields0;
    AfterRecRead( pRec );
    //fFields0.Rec := fTempRec.Rec;
    //fFields1.Rec := fTempRec.Rec;
    fFields0 := +fTempRec;
    fFields1 := +fTempRec;
  end
  else if pRec = fFields2RecNo then
...
begin
  fInterestedParties.Lock;
  try
    fInterestedParties.Remove( pParty );
    fInterestedParties.Pack;
  finally
    fInterestedParties.Unlock
  end;
end;
...
constructor TInterestedPartiesList.Create;
begin
  inherited Create;
  FLock := TObject.Create;
end;

destructor TInterestedPartiesList.Destroy;
begin
  FLock.Free;
  inherited;
end;

procedure TInterestedPartiesList.Lock;
begin
  TMonitor.Enter( FLock );
end;
...
procedure TInterestedPartiesList.Unlock;
begin
  TMonitor.Exit( FLock );
end;

{ TSigDBFileBase<T>.tD2 }

class operator TSigDBFileBase<T>.tD2.Positive(pSrc: tD2): tD2;
begin
  Result.Rec := pSrc.Rec;
  if assigned( pSrc.UserData ) then
  begin
    //Result.UserData := pSrc.UserData.Clone( pSrc.UserData );
    Result.UserData := pSrc.UserData.Clone;
  end
  else
  begin
    Result.UserData := nil;
  end;
  //CopyUserData( Result.UserData, pSrc.UserData );
end;
