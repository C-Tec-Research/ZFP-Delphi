...
type
  ...
  public
    ...
    function OwnerAs< pType : class > : pType;
...
{$ELSE}
    ...
    function SaveStructure( pTreeNodes : TTreeNodes; pTreeNode : TTreeNode ) : TTreeNode; overload; virtual;
{$ENDIF}
    ...
    procedure MakeMeActive(const pCaller : TSigBaseProperty = nil ); virtual;
              // Makes both the object and its parents active
...
{$ELSE}
    ...
    function SaveStructure( pTreeNodes : TTreeNodes; pTreeNode : TTreeNode ) : TTreeNode; overload;
{$ENDIF}
...
{$ENDIF}
  ...
  public
    ...
    function AddObject( const pString : string; const pObject : TObject ) : integer;
  ...
  public
    procedure ClearUndoList( SendMessage : boolean = TRUE ); virtual;
    constructor Create( pPropertyName : string; pOwner : tSigCompoundProperty  ); override;
    destructor Destroy; override;
...
{$ELSE}
    ...
    function SaveStructure( pTreeNodes : TTreeNodes; pTreeNode : TTreeNode ) : TTreeNode; overload; override;
{$ENDIF}
    procedure CopyToClipboard( pClipboard : tStrings; pShortFormat : boolean = FALSE; pIndent : integer = 0  ); override;
    function Load( pFile : tStrings; var pLine : integer; const pAllowUndo : boolean; const pIsDirty : boolean; pErrors : tErrorList = nil ) : boolean; override;
    function PasteFromClipboard( pClipboard : tStrings; var pLine : integer; pErrors : tErrorList = nil ) : boolean; override;
    ...
    procedure MakeMeActive(const pCaller : TSigBaseProperty = nil ); override;
              // Makes both the object and its parents active
...
{$IF CompilerVersion >= 30}
  TSigArrayProperty< T : TSigBaseProperty > = class( TSigArrayProperty )
  private
    function GetItemT(const i: integer): T;
  public
    property Item[ const i : integer ] : T
             read GetItemT; default;
    function CreateChild( const pPropertyText : string; const pIndexText : string;
                          const pValue : string = ''; const pComment : string = '';
...
{$ELSE}
    ...
    function GetClearRemovesChildren: boolean;
    procedure SetClearRemovesChildren(const Value: boolean);
{$ENDIF}
  protected
...
{$ENDIF}
    ...
    property ClearRemovesChildren : boolean
             read GetClearRemovesChildren
             write SetClearRemovesChildren;
...
{$ENDIF}
    ...
    fUsesDeviceMenu: boolean;
    fUsesHelpMenu: boolean;
...
{$ENDIF}
    ...
    function GetUsesEditMenu: boolean;
  protected
  ...
  public
    LoadVersion : TSigFileVer;
    SaveVersion : TSigFileVer;
...
{$ELSE}
    ...
    function SaveStructure( pTreeNodes : TTreeNodes ) : TTreeNode; reintroduce; overload;
{$ENDIF}
...
{$ENDIF}
    property UsesEditMenu : boolean
             read GetUsesEditMenu
             write fUsesEditMenu;
    property UsesUndoRedoMenu : boolean
             read GetUsesUndoRedoMenu
             write fUsesUndoRedoMenu;
    property UsesDeviceMenu : boolean
             read fUsesDeviceMenu
             write fUsesDeviceMenu;
    property UsesHelpMenu : boolean
             read fUsesHelpMenu
             write fUsesHelpMenu;
...
procedure TSigBaseProperty.MakeMeActive(const pCaller: TSigBaseProperty);
begin
  if assigned( Owner ) then
  begin
    Owner.MakeMeActive( self );
  end;
  // For simple objects (that have no children) the parameter pCaller is meaningless
end;
...
function TSigBaseProperty.OwnerAs<pType>: pType;
var
  iResult : TSigCompoundProperty;
begin
  iResult := Owner;
  while assigned( iResult ) do
  begin
    if iResult is pType then
    begin
      Result := iResult as pType;
      exit;
    end;
    // else
    iResult := iResult.Owner;
  end;
...
function TSigBaseProperty.SaveStructure(pTreeNodes: TTreeNodes;
...
{$IF CompilerVersion = 28}
  if Addr(fSigOnChange) <> Addr(Value) then
{$ELSE}
    ...
    begin
      ...
      begin
        try
          ...
          (iItem as TSigCompoundProperty).ClearUndoList( FALSE );
        except
           raise Exception.Create('Problem with ' + iItem.fPropertyName);
        end;
      end;
    end;
    if SendMessage then
...
function TSigPropertyList.SaveStructure(pTreeNodes: TTreeNodes;
  pTreeNode: TTreeNode): TTreeNode;
var
  i, j : Integer;
  iText : string;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if Indexed then
            begin
              for j := 0 to iArrays.Count - 1 do
              begin
                if iArrays[ j ] = PropertyName then
                begin
                  iIsDup := TRUE;
                  break;   // only single entity for array - event with text indices!
                end;
              end;
              if not iIsDup then
              begin
                iArrays.Add( PropertyName );
                iText := iText + '( array )';
              end;
            end;
            if iIsDup then
            ...
            begin
              Result := pTreeNodes.AddChildObject( pTreeNode, iText, Item[ i ] );
              SaveStructure( pTreeNodes, Result );
            end;
          end;
        end;
      end; // ignore dodgy ones!
...
procedure TSigCompoundProperty.ClearUndoList(SendMessage: boolean);
begin
  Children.ClearUndoList( SendMessage );
end;
...
procedure TSigCompoundProperty.MakeMeActive(const pCaller: TSigBaseProperty);
var
  i: Integer;
begin
  inherited;
...
begin
  ...
  begin
    ...
    else if self is TSigFileProperty then
    begin
      iVersion := (self as TSigFileProperty).SaveVersion;
      if iVersion <> '' then
      begin
        iLine := iLine + '( ' + iVersion + ' )';
      end;
    end;
    if Value <> '' then
    begin
...
{$IFDEF FMX_SIGFILE}
{$ELSE}
function TSigCompoundProperty.SaveStructure(pTreeNodes: TTreeNodes;
  pTreeNode: TTreeNode): TTreeNode;
begin
  result := Children.SaveStructure( pTreeNodes, pTreeNode );
end;
{$ENDIF}
...
function TMemoProperty.GetText: string;
begin
...
procedure TMemoProperty.RemoveEditors;
begin
  inherited;
  Editor := nil;
end;
...
procedure TMemoProperty.SetText(const Value: string);
begin
  if fStrings.Text <> Value then
  begin
    fStrings.Text := Value;
    IsDirty := TRUE;
  end;
...
function TSigFileProperty.GetUsesEditMenu: boolean;
begin
  Result := fUsesEditMenu;
end;
...
{$IFDEF FMX_SIGFILE}
{$ELSE}
function TSigFileProperty.SaveStructure(pTreeNodes: TTreeNodes): TTreeNode;
var
  iText : string;
begin
  pTreeNodes.Clear;
  ...
  Result := pTreeNodes.AddChildObject( nil, iText, self );
  Result := inherited SaveStructure( pTreeNodes, Result );
end;
{$ENDIF}
...
procedure TSigBooleanProperty.RemoveEditors;
begin
  inherited;
  Editor := nil;
end;
...
procedure TSigIntegerProperty.RemoveEditors;
begin
  inherited;
  Editor := nil;
end;
...
function tSigObjectList.GetClearRemovesChildren: boolean;
begin
  Result := ChildArray.ClearRemovesChildren
end;
...
procedure tSigObjectList.SetClearRemovesChildren(const Value: boolean);
begin
  ChildArray.ClearRemovesChildren := Value;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      StringListControlList.Items[ pItem ].AddObject(  Entry[ i ].StringListText, Entry[ i ] );
    end;
...
procedure tSigObjectArray.RemoveEditors;
begin
  inherited;
  CountEditor := nil;
  MaxEditor := nil;
  RecEditor := nil;
  StringListControl := nil;
end;
...
begin
  ...
  else if Value is TListBox then
...
procedure TSigTextProperty.RemoveEditors;
begin
  inherited;
  Editor := nil;
end;
...
procedure tSigEnumProperty.RemoveEditors;
begin
  inherited;
  Editor := nil;
end;
...
procedure TSigEnum<T>.RemoveEditors;
begin
  inherited;
  RadioGroup := nil;
  ComboBox := nil;
...
procedure tSigEnumSet<T>.RemoveEditors;
begin
  inherited;
  Editor := nil;
end;
...
procedure TSigStringArray.RemoveEditors;
begin
  inherited;
  Editor := nil;
end;
...
procedure TSigEnumMapList.RemoveEditors;
begin
  inherited;
  MaxEditor := nil;
  CountEditor := nil;
...
procedure tSigMappedEnum.RemoveEditors;
begin
  inherited;
  Editor := nil;
end;
...
destructor tSigPointer.Destroy;
begin
  //fDestinationList.Free;  We don't own this!
...
procedure tSigMappedObject.RemoveEditors;
begin
  inherited;
...
begin
  //inherited Create( TRUE );
  inherited Create( FALSE ); // we never crea our children - they are always copies
...
function tStringListControl.AddObject(const pString: string;
  const pObject: TObject): integer;
begin
  Result := fStrings.AddObject( pString, pObject );
end;
...
function TSigArrayProperty<T>.CreateChild(const pPropertyText, pIndexText,
  pValue, pComment: string; pErrors: tErrorList; pErrorLine,
  pErrorPos: integer): tSigBaseProperty;
begin
  if SameText( pPropertyText, 'Item') then
  begin
    Result := T.Create( pPropertyText, pIndexText, self );
    if pValue <> '' then
    begin
      Result.Value := pValue;
    end;
...
function TSigArrayProperty<T>.GetItemT(const i: integer): T;
begin
  Result := Entry[ i ] as T;
end;
