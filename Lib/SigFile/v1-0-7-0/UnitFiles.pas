...
type
  tSigDBBComparisonStyle = ( cs_None, cs_Numeric, cs_Enum, cs_Text, cs_String, cs_DateTime, cs_Set_Membership );
...
begin
  ...
  begin
    Add('    property ' + fFieldName.Value + ' : ' + fFieldType.Value);
    Add('             read fFields0.Rec.Data2.' + fFieldName.Value);
    Add('             write  fFields0.Rec.Data2.' + fFieldName.Value + ';');
  end;
end;
...
begin
  ...
  begin
    ...
    case self.fCompareStyle.ValueAsStyle of
      ...
      cs_Text:           iLine := iLine + 'CompareText( pRec1.' + fFieldName.Value + ', pRec2.' + fFieldName.Value +' );';
      cs_String:         iLine := iLine + 'CompareStr( pRec1.' + fFieldName.Value + ', pRec2.' + fFieldName.Value +' );';
...
begin
  ...
  begin
    ...
    case self.fCompareStyle.ValueAsStyle of
      ...
      cs_Enum:           iLine := iLine + 'Ord(pRec.' + fFieldName.Value + ') - Ord( pFilter );';
      cs_Text:           iLine := iLine + 'CompareText( pRec.' + fFieldName.Value + ', pFilter );';
      cs_String:         iLine := iLine + 'CompareStr( pRec.' + fFieldName.Value + ', pFilter );';
      cs_DateTime:       iLine := iLine + 'CompareDateTime( pRec.' + fFieldName.Value + ', StrToDateTime( pFilter ));';
      cs_Set_Membership: iLine := 'raise exception.Create(''Invalid Set Comparison with Text'');'; // this form not valid in data files
    end;
    Add( iLine );
  end;
end;
...
begin
  ...
  begin
    ...
    Add('  public');
    Add('');
    Add('    function OpenDBFile( const pVerMajor, pVerMinor : Word ) : boolean; override;');
    Add('');
...
begin
  ...
  begin
    ...
    Add( '' );
    Add( 'function T' + fBaseFileName.Value + 'Data.OpenDBFile(const pVerMajor,');
    Add( '  pVerMinor: Word): boolean;');
    Add( 'begin');
    Add( '  Result := inherited;');
    Add( 'end;');
    Add( '');
...
function tSigDBBFieldPointer.GetCelText: string;
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          Add( '    Add( tSigDBIndexField.Create( Fields, ' + Filter.ValueToTypeText + ', T'+ DataFile.BaseFileName.Value +'Data.c' + DBBField.DBBField.FieldName.Value + ' ));' );
        end
        else
        begin
...
begin
  for i := 0 to Max do
  begin
    SigDBBIndexFile[ i ].AddClassDef( pLines );
  end;
end;
...
begin
  for i := 0 to Max do
  begin
    SigDBBIndexFile[ i ].AddClassImplementation( pLines );
  end;
end;
...
begin
  ...
  iFull := '  ' + iTest +  ' := T' + FullName + '.Create( ''' + DataFile.BaseFileName.Value +''', self, DataFiles.' + DataFile.BaseFileName.Value + 'Data );';
  while i < pLines.Count do
  begin
    if Pos( 'f' + FullName, pLines[ i ] ) <> 0 then
    begin
...
begin
  ...
  begin
    ...
    Add( iLine + IndexFile.KeyParm( FindLevel.ValueAsInt -1 ) + ' ) : tSigDBRecPointer; overload;' );
    Add( '' );
  end;
end;
