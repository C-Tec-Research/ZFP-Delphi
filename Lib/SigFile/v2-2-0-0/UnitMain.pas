...
uses
  ...
  Vcl.CheckLst,
  SigVariableEditorList, System.ImageList;
...
type
  TFormMain = class(TForm)
    ...
    TabSheetFileProperties: TTabSheet;
    ...
    TabSheetFindFunctions: TTabSheet;
    ...
    procedure EditFileBaseNameChange(Sender: TObject);
    procedure EditIndexExtensionChange(Sender: TObject);
    procedure SpeedButtonDeleteIndexFileClick(Sender: TObject);
    procedure TreeViewDatabaseKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
  private
...
procedure TFormMain.EditFileBaseNameChange(Sender: TObject);
begin
  if assigned( TreeViewDatabase.Selected ) then
  begin
    if PageControlProperties.Visible then
    begin
      if PageControlProperties.ActivePage = TabSheetFileProperties then
      begin
        TreeViewDatabase.Selected.Text := EditFileBaseName.Text;
      end;
    end;
  end;
end;
...
procedure TFormMain.EditIndexExtensionChange(Sender: TObject);
var
  iIndexFile : TSigDBBIndexFile;
begin
  if assigned( TreeViewDatabase.Selected ) then
  begin
    if PageControlProperties.Visible then
    begin
      if PageControlProperties.ActivePage = TabSheetIndexFiles then
      begin
        iIndexFile := TreeViewDatabase.Selected.Data;
        TreeViewDatabase.Selected.Text := iIndexFile.FullName;
      end;
    end;
  end;
end;
...
begin
  ...
  fSigFile.BLOBDirEditor := EditBLOBDir;
  fSigFile.LASTDirEditor := EditLASTDir;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if assigned( TreeViewDatabase.Selected ) then
          begin
            TreeViewDatabase.Items.Delete( TreeViewDatabase.Selected );
          end;
        end;
      end;
    end;
...
procedure TFormMain.SpeedButtonDeleteIndexFileClick(Sender: TObject);
var
  iNode : TTreeNode;
  iNext : TTreeNode;
  iObject : TObject;
begin
  case MessageDlg( 'This will delete the index file and all associated search functions. Are you sure?', mtWarning,
                   [mbYes, mbCancel], 0) of
    mrYes:
    begin
      with fSigFile.DataBase.Files do
      begin
        if ActiveChild > 0 then
        begin
          with BaseFile[ ActiveChild ].IndexFiles do
          begin
            Delete( ActiveChild, undoDelete2 );
            iNode := TreeViewDatabase.Selected;
            if assigned( iNode ) then
            begin
              iNext := iNode.getNextSibling;
              iObject := iNext.Data;
              if assigned( iObject ) then
              begin
                if iObject is tSigDBBIndexFileList then
                begin
                  TreeViewDatabase.Selected := iNode.getPrev;
                end;
              end;
              TreeViewDatabase.Items.Delete( iNode );
            end;
          end;
        end;
      end;
...
procedure TFormMain.TreeViewDatabaseKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  Key := 0;
  // ignore key presses!
