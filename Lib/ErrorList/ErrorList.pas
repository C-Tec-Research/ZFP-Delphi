{$IFNDEF VCL_ERRORLIST}
{$IFNDEF FMX_ERRORLIST}
unit ErrorList;
{$ENDIF}
{$ENDIF}
...
type
  ...
  public
    ...
    procedure ClearWarningsAndHints;
...
procedure TErrorList.ClearHints;
var
  i: Integer;
begin
  for i := Count - 1 downto 0 do
  begin
    if Error[ i ].Severity = es_Hint then
    begin
      Delete( i );
      dec( fHintCount );
    end;
...
procedure TErrorList.ClearWarningsAndHints;
var
  i: Integer;
begin
  for i := Count - 1 downto 0 do
  begin
    case Error[ i ].Severity of
      es_Warning:
      begin
        Delete( i );
        dec( fWarningCount );
