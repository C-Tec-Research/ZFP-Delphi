...
type
  ...
  protected
    ...
    iCopyID : integer;
    isInTimer : boolean;
    procedure InternalTimerAction (Sender: TObject); override;
    { we need to intercept the OnAck, OnNak and OnTimeOut events}
    procedure InternalOnAck(Sender: TObject); override;
    procedure InternalOnNak(Sender: TObject); override;
...
procedure TSS2NCommsPanel.InternalTimerAction (Sender: TObject);
var
...
{        form1.memo1.lines.add (text);}
  ...
  inherited InternalTimerAction( Sender );
...
procedure TSS2NCommsPanel.InternalOnAck(Sender: TObject);
begin
  if  not isClosing
  and not iCanProcessMessage
...
procedure TSS2NCommsPanel.InternalOnNak(Sender: TObject);
begin
  if  not isClosing
  and not iCanProcessMessage
  and (iActiveMessage > 0)
