...
type
  ...
  tSigFileUndoAction = ( undoClearUndoList, undoValue, undoChangeMax, undoCreate, undoInsert, undoDelete, undoSelPos, undoSelLen,
                         undoChangeActiveChild, undoMoveFrom, undoMoveTo, undoSwap );
  ...
  private
    ...
    fOnSwapChild: tOnArrayChildMove; // temporaries to allow undo/redo actions
  ...
  public
    ...
    procedure Swap( const pFrom, pTo : integer );
...
procedure tSigArrayProperty.Swap(const pFrom, pTo: integer);
var
  iSave : tObject;
begin
...
procedure tSigObjectArray.Swap(const pFrom, pTo: integer);
var
  iSave : tObject;
begin
  if assigned( fOnSwapChild ) then
...
begin
  if SigNETParse ( fValue, SigProperty,
           SigIndex, SigValue, SigComment) then
  begin
    if assigned( fDestinationList ) then
    begin
