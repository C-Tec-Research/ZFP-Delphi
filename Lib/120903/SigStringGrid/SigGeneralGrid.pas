...
type
  ...
  published
    ...
    procedure ResetText( const pRow : integer; const pNewVal : string );
  end;
    ...
    procedure SetCellObject(const Col, Row: integer; const Value: tObject);
{
  ...
  private
    ...
    fOnCellEditClick: tOnCellChange;
    function GetCell(const Col, Row : integer): string;
    procedure SetCell(const Col, Row : integer; const Value: string);
  ...
  published
    ...
    property OnCellEditClick : tOnCellChange   // only called when an editor changes a value
             read fOnCellEditClick
             write fOnCellEditClick;
...
begin
  ...
  if assigned( iEditor ) then
  begin
    iEditor.DrawCell( Canvas, ACol, ARow, Rect, State, Value, OwnerEnabled, OwnerFocused, CellEnabled, ErrorText);
  end
  else
  begin
...
begin
  Editor[ ACol ].ResetText( ARow, pNewVal );
end;
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                if (fCellRowBeingEdited <> ARow) or (fCellColBeingEdited <> ACol ) then
                begin
                  SigGrid.InvalidateCell( fCellColBeingEdited, fCellRowBeingEdited );
            ...
            begin
              ...
              begin
                ...
                fSigGrid.InvalidateCell( fCellColBeingEdited, fCellRowBeingEdited );
  ...
  case fStyle of
    ...
    begin
      ...
      begin
        if assigned( OnGetImageIndex) then
        begin
          iImageIndex := OnGetImageIndex( ACol, ARow, [], iValue );
        end
        else if iValue = '' then
        begin
          iImageIndex := 0;
...
procedure tSigGridEditor.ResetText( const pRow : integer; const pNewVal: string);
begin
  if pRow = CellRowBeingEdited then
  begin
    if assigned( fEditor ) then
    begin
