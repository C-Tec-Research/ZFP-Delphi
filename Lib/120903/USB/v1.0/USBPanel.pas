        ...
        begin
          ...
          begin
            ...
            begin
              ...
              begin
                Cells[ cIsoOut, i + 1 ] := 'Enabled';
              end
              else
              begin
                Cells[ cIsoOut, i + 1 ] := 'Disabled';
              end;
...
begin
  ...
  begin
    ...
    begin
      if GetFTDeviceDescription( i ) = Device_String then
      begin
        if iOpen then
        begin
          Close_USB;
        ...
        try
          if FT_Error_Check( 'Opening USB', FT_OpenEx( @iDevice_String, FT_OPEN_BY_SERIAL_NUMBER, @iFT_Handle )) then
          begin
            iOpen := TRUE;
            fSetCurrentBaudRate;
...
begin
  ...
  begin
    if FT_Error_Check( 'Getting USB Device Description for device ' + IntToStr( DevNo ),
       FT_ListDevices( DevNo, @Buffer, FT_OPEN_BY_SERIAL_NUMBER or FT_LIST_BY_INDEX )) then
    begin
      for i := 1 to 50 do
      begin
...
begin
  ...
  begin
    ...
    FT_Error_Check( 'writing to USB port', FT_Write( iFT_Handle, @self.iWriteBuffer, Length( NewVal ), @rPtr ));
  end;
end;
...
var
  pending : dword;
  rcvd : dword;
  RcvdChar : array [ 1..2 ] of char; // leave one spare in case
begin
  if not ( csDesigning in ComponentState ) then
  begin
      ...
      try
        ...
        begin
          ...
          begin
            ...
            iReadBuffer[ iRcvUSBBufferCount ] := RcvdChar[ 1 ];
            inc( iRcvUSBBufferCount );
            if assigned( iOnChar ) then
            begin
              iOnChar( self, RcvdChar[ 1 ] );
            end;
...
begin
  ...
  begin
    if assigned( FT_EE_Read ) then
    begin
      Result := FT_Error_Check( 'reading USB device EEPROM',  FT_EE_Read( iFT_Handle, iProgramData ));
    end;
  end;
end;
