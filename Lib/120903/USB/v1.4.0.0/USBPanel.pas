...

{
    ...
    iActualManufacturer : array [ 0..50 ] of AnsiChar;
    iActualManufacturerID : array [ 0..50 ] of AnsiChar;
    iActualDescription : array [ 0..64 ] of AnsiChar;
    iActualSerialNo : array [ 0..50 ] of AnsiChar;

    // values set up by user - used for checking by open method
    iProgramProgramData : FT_PROGRAM_DATA;
    iProgramManufacturer : array [ 0..50 ] of AnsiChar;
    iProgramManufacturerID : array [ 0..50 ] of AnsiChar;
    iProgramDescription : array [ 0..64 ] of AnsiChar;
    iProgramSerialNo : array [ 0..50 ] of AnsiChar;

    iRcvUSBBufferCount : integer;
    iReadBuffer : array of AnsiChar;
    iWriteBuffer : array of AnsiChar;
    ...
    // Auto Open
    iAutoOpen : boolean;
    iOnOpen : tNotifyEvent;
    iOnClose : tNotifyEvent;

    // Auto program
    iAutoProgram : boolean;
    iOnBlankEEProm : tNotifyEvent; // occurs if blank EEPROM found, before programming
    iOnProgram : tNotifyEvent; // occurs after programming
    iDetectedDeviceCount : integer;

    iOnErr : TOnErr;

    procedure fSetOpen( NewVal : boolean );
    procedure fSetTimeOut( NewVal : integer );
...
//    procedure fSetImage( pImage: TImage; pRangeMax : integer; pImageNo : integer );
    ...
    procedure fSetAutoOpen( NewVal : boolean );
    procedure fSetAutoProgram( NewVal : boolean );

    procedure fSetDetectedDeviceCount( NewVal : integer );
    procedure fCheckStillOpen;

{
  ...
  public
    ...
    property DetectedDeviceCount : integer
             read iDetectedDeviceCount
             write fSetDetectedDeviceCount;
...
//    iAutoConnect : boolean;
    ...
    iDefaultManufacturer : array [ 0..50 ] of AnsiChar;
    iDefaultManufacturerID : array [ 0..50 ] of AnsiChar;
    iDefaultDescription : array [ 0..64 ] of AnsiChar;
    iDefaultSerialNo : array [ 0..50 ] of AnsiChar;
...
begin
  ...
  begin
    ...
    begin
      Result := Result + char(iDefaultManufacturer[ i ]);
    end;
  end;
end;
...
begin
  ...
  begin
    iDefaultProgramData.Manufacturer[ i - 1 ] := AnsiChar( NewVal[ i ] );
  end;
...
begin
  ...
  begin
    ...
    begin
      Result := Result + char(iDefaultManufacturerID[ i ]);
    end;
  end;
end;
...
begin
  ...
  for i := 1 to length( NewVal ) do
  begin
    iDefaultProgramData.ManufacturerID[ i - 1 ] := AnsiChar( NewVal[ i ] );
  end;
...
begin
  ...
  begin
    ...
    begin
      Result := Result + char(iDefaultDescription[ i ]);
    end;
  end;
end;
...
begin
  ...
  for i := 1 to length( NewVal ) do
  begin
    iDefaultDescription[ i - 1 ] := AnsiChar( NewVal[ i ] );
  end;
...
begin
  ...
  begin
    ...
    begin
      Result := Result + char(iDefaultSerialNo[ i ]);
    end;
  end;
end;
...
begin
  ...
  for i := 1 to length( NewVal ) do
  begin
    iDefaultSerialNo[ i - 1 ] := AnsiChar( NewVal[ i ] );
  end;
        ...
        begin
          ...
          begin
            ...
            case iPortStatus of
              ...
              Cells[ cManufacturer, i + 1 ] := string(iActualProgramData.Manufacturer);
              Cells[ cManufacturerID, i + 1 ] := string(iActualProgramData.ManufacturerID);
              Cells[ cDescription, i + 1 ] := string(iActualProgramData.Description);
              if assigned( iActualProgramData.SerialNo ) then
              begin
                Cells[ cSerialNo, i + 1 ] := string(iActualProgramData.SerialNo);
              end
              else
              begin
...
begin
  ...
  begin
    case PortStatus of
      ...
      FT_UNFORMATTED_EEPROM : Str := 'Error ' + ErrStr + ' - Unformatted device...'; // deduced!
      else
        Str := 'Error ' + ErrStr + ' - Undefined Error ' + IntToStr( PortStatus );
    end;
    result := FALSE;
    pHandled := FALSE;
    if assigned( iOnErr ) then
    begin
      iOnErr( self, PortStatus, Str, pHandled );
    end;
...

{
  ...
  iDefaultProgramData.Product_ID     := $6001;
  iDefaultProgramData.Manufacturer   := @iDefaultManufacturer;
  StrCopy( @iDefaultManufacturer, 'FTDI' );
  iDefaultProgramData.ManufacturerID := @iDefaultManufacturerID;
  StrCopy( @iDefaultManufacturerID, 'FT' );
  iDefaultProgramData.Description    := @iDefaultDescription;
  StrCopy( @iDefaultDescription, 'USB HS Serial Converter');
  iDefaultProgramData.SerialNo       := @iDefaultSerialNo;
  StrCopy( @iDefaultSerialNo, 'FT000001' );
  iDefaultProgramData.MaxPower       := 44;
  iDefaultProgramData.PnP            := 1;
  iDefaultProgramData.SelfPowered    := 0;
  iDefaultProgramData.RemoteWakeup   := 1;
  ...
  iProgramProgramData.Vendor_ID      := $0403;
  iProgramProgramData.Product_ID     := $6001;
  iProgramProgramData.Manufacturer   := @iProgramManufacturer;
  StrCopy( @iProgramManufacturer, 'FTDI' );
  iProgramProgramData.ManufacturerID := @iProgramManufacturerID;
  StrCopy( @iProgramManufacturerID, 'FT' );
  iProgramProgramData.Description    := @iProgramDescription;
  StrCopy( @iProgramDescription, 'USB HS Serial Converter');
  iProgramProgramData.SerialNo       := @iProgramSerialNo;
  StrCopy( @iProgramSerialNo, 'FT000001' );
  iProgramProgramData.MaxPower       := 44;
  iProgramProgramData.PnP            := 1;
  iProgramProgramData.SelfPowered    := 0;
  iProgramProgramData.RemoteWakeup   := 1;
    //
    // Rev 4 extensions
    //
  iProgramProgramData.Rev4           := FALSE;  // True if rev4 chip, false otherwise
  iProgramProgramData.IsoIn          := FALSE;  // true if endpoint is isosynchronous
  iProgramProgramData.IsoOut         := FALSE;  // true if endpoint is isosynchronous
  iProgramProgramData.PullDownEnable := FALSE;  // true if Pull Down enabled
  iProgramProgramData.SerNumEnable   := FALSE;  // True if serial number to be used
  iProgramProgramData.USBVersion     := $0200; // BCD (0x0200 => USB 2.0)
...
//  iImageList := nil;
  ...
  iTimer := TTimer.Create( self );
//  iTimer := TTimer.Create( AOwner );
//  iTimer := TTimer.Create( nil );
  iTimer.Enabled := not (csDesigning in ComponentState );
  TickInterval := 10;
  iTimer.OnTimer := fTimerAction;
...
procedure TUSBPanel.fSetTimeOut( NewVal : integer );
begin
  if iTimeOut <> NewVal then
  begin
    iTimeOut := NewVal;
    if iTimeOutCountdown > iTimeOut then
    begin
      iTimeOutCountdown := iTimeOut;
    end;
  end;
...
procedure TUSBPanel.fCheckStillOpen;
var
  iPortStatus : integer;
begin
  if iOpen then
  begin
    iPortStatus := FT_EE_Read( iFT_Handle, @iActualProgramData );
...
begin
  ...
  // call iOnOpen if we succeeded
  if iOpen and assigned( iOnOpen ) then
  begin
    iOnOpen( self );
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      try
        iPortStatus := FT_Open( i, @iFT_Handle );
        if (iPortStatus = FT_UNFORMATTED_EEPROM)
        or (iPortStatus = FT_INVALID_HANDLE) then
        begin
          if iAutoProgram then
          begin
            // need to format EEPROM
            if assigned( FT_EE_Program ) then
            begin
              iPortStatus := FT_EE_Program( iFT_Handle, @iProgramProgramData );
            end;
          end;
        end;
        ...
        begin
          if Read_Device then
          begin
            if (iProgramProgramData.Vendor_ID = iActualProgramData.Vendor_ID)
                 and (iProgramProgramData.Product_ID = iActualProgramData.Product_ID) then
            begin
              if (iOpenMethod = omByID) or (AnsiStrComp( iProgramSerialNo, iActualSerialNo ) = 0) then
              begin
                // done
                iOpen := TRUE;
                iDeviceNo := i;
...
begin
  ...
  if iOpen then
  begin
    if iActualProgramData.PnP = 0 then Result := PnP_disabled
    else Result := PnP_enabled;
  end
  else
  begin
    if iProgramProgramData.PnP = 0 then Result := PnP_disabled
    else Result := PnP_enabled;
  end
end;
...
begin
  iProgramProgramData.PnP := Ord( NewVal );
end;
...
begin
  ...
  begin
    Result := Result + char(iReadBuffer[ i ]);
  end;
  ...
  begin
    ...
    begin
      Result := FT_Error_Check( 'programming USB device',
                FT_EE_Program( iFT_Handle, @iProgramProgramData ));
    end;
  end;
end;
...
begin
  ...
  begin
    for i := 1 to length( NewVal ) do
    begin
      iWriteBuffer[ iWriteBufferPending  ] := AnsiChar(NewVal[ i ]);
...
//    FT_Error_Check( 'writing to USB port', FT_Write( iFT_Handle, @iWriteBuffer, Length( NewVal ), @rPtr ));
  ...
  if assigned( iSendBufferImage ) then
  begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if pending > 0 then
        begin
          if assigned( iRcvBufferImage ) then
          begin
        ...
        begin
          ...
          if FT_Error_Check('Reading from USB Port', FT_Read( iFT_Handle, @RcvdChar, 1, @rcvd )) then
          begin
...
procedure TUSBPanel.fSetAutoOpen( NewVal : boolean );
begin
  if iAutoOpen <> NewVal then
  begin
    iAutoOpen := NewVal;
    if not NewVal then
    begin
...
procedure TUSBPanel.fSetAutoProgram( NewVal : boolean );
begin
  if iAutoProgram <> NewVal then
  begin
    iAutoProgram := NewVal;
    if NewVal then
    begin
...
procedure TUSBPanel.fSetDetectedDeviceCount( NewVal : integer );
begin

  if iDetectedDeviceCount <> NewVal then
  begin
    if iDetectedDeviceCount < NewVal then
    begin
      iDetectedDeviceCount := NewVal;
      // may be new device. If iAutoOpen set to TRUE, and we are not
    ...
    begin
      ...
      if iDetectedDeviceCount = 0 then // no need to check
      begin
        Close_USB;
      end
      else
...
begin
  ...
  begin
    ...
    begin
      iPortStatus := FT_EE_Read( iFT_Handle, @iActualProgramData );
      if iPortStatus = FT_UNFORMATTED_EEPROM then
      begin
        if iAutoProgram then
        begin
          if assigned( FT_EE_Program ) then
...
begin
  if iOpen then
  begin
    Result := IntToHex( iActualProgramData.Vendor_ID, 4 );
  end
  else
  begin
    Result := IntToHex( iProgramProgramData.Vendor_ID, 4 );
  end;
end;

procedure TUSBPanel.fSetVendorID( NewVal : string );
begin
  iProgramProgramData.Vendor_ID := StrToInt( '$' + NewVal );
end;
...
begin
  if iOpen then
  begin
    Result := IntToHex( iActualProgramData.Product_ID, 4 );
  end
  else
  begin
    Result := IntToHex( iProgramProgramData.Product_ID, 4 );
  end;
end;

procedure TUSBPanel.fSetProductID( NewVal : string );
begin
  iProgramProgramData.Product_ID := StrToInt( '$' + NewVal );
end;
...
begin
  if iOpen then
  begin
    StrCopy( iManufacturer, iActualManufacturer );
  end
  else
  begin
    StrCopy( iManufacturer, iProgramManufacturer );
  end;
  Result := '';
...
begin
  ...
  for i := 1 to length( NewVal ) do
  begin
    iProgramProgramData.Manufacturer[ i - 1 ] := AnsiChar( NewVal[ i ] );
  end;
  iProgramProgramData.Manufacturer[ length( NewVal ) ] := #0;
end;
...
begin
  if iOpen then
  begin
    StrCopy( iManufacturerID, iActualManufacturerID );
  end
  else
  begin
    StrCopy( iManufacturerID, iProgramManufacturerID );
  end;
  Result := '';
...
begin
  ...
  for i := 1 to length( NewVal ) do
  begin
    iProgramProgramData.ManufacturerID[ i - 1 ] := AnsiChar( NewVal[ i ] );
  end;
  iProgramProgramData.ManufacturerID[ length( NewVal ) ] := #0;
end;
...
begin
  if iOpen then
  begin
    StrCopy( iDescription, iActualDescription );
  end
  else
  begin
    StrCopy( iDescription, iProgramDescription );
  end;
  Result := '';
...
begin
  ...
  for i := 1 to length( NewVal ) do
  begin
    iProgramDescription[ i - 1 ] := AnsiChar(NewVal[ i ]);
  end;
  iProgramDescription[ length( NewVal ) ] := #0;
end;
...
begin
  ...
  if iOpen then
  begin
    StrCopy( iSerialNo, iActualSerialNo );
  end
  else
  begin
    StrCopy( iSerialNo, iProgramSerialNo );
  end;
...
begin
  ...
  for i := 1 to length( NewVal ) do
  begin
    iProgramSerialNo[ i - 1 ] := AnsiChar(NewVal[ i ]);
  end;
  iProgramSerialNo[ length( NewVal ) ] := #0;
end;
...
begin
  if iOpen then
  begin
    Result := iActualProgramData.MaxPower;
  end
  else
  begin
    Result := iProgramProgramData.MaxPower;
  end;
end;
...
begin
  if (NewVal > 0) and (NewVal <= 500) then
  begin
    iProgramProgramData.MaxPower := NewVal;
  end
  else
  begin
    raise Exception.Create('USB Max power must lie in range 1-500' );
...
begin
  if iOpen then
  begin
    if iActualProgramData.SelfPowered = 0 then Result := spBusPowered
    else Result := spSelfPowered;
  end
  else
  begin
    if iProgramProgramData.SelfPowered = 0 then Result := spBusPowered
    else Result := spSelfPowered;
  end;
end;
...
begin
  iProgramProgramData.SelfPowered := Ord( NewVal );
end;
...
begin
  if iOpen then
  begin
    if iActualProgramData.RemoteWakeup = 0 then Result := rwuNotCapable
    else Result := rwuCapable;
  end
  else
  begin
    if iProgramProgramData.RemoteWakeup = 0 then Result := rwuNotCapable
    else Result := rwuCapable;
  end;
end;
...
begin
  iProgramProgramData.RemoteWakeup := Ord( NewVal );
end;
...
begin
  if iOpen then
  begin
    Result := iActualProgramData.Rev4;
  end
  else
  begin
    Result := iProgramProgramData.Rev4;
  end;
end;

procedure TUSBPanel.fSetRev4( NewVal : boolean );
begin
  iProgramProgramData.Rev4 := NewVal;
end;
...
begin
  if iOpen then
  begin
    Result := iActualProgramData.IsoIn;
  end
  else
  begin
    Result := iProgramProgramData.IsoIn;
  end;
end;

procedure TUSBPanel.fSetIsoIn( NewVal : boolean );
begin
  iProgramProgramData.IsoIn := NewVal;
end;
...
begin
  if iOpen then
  begin
    Result := iActualProgramData.IsoOut;
  end
  else
  begin
    Result := iProgramProgramData.IsoOut;
  end;
end;

procedure TUSBPanel.fSetIsoOut( NewVal : boolean );
begin
  iProgramProgramData.IsoOut := NewVal;
end;
...
begin
  if iOpen then
  begin
    Result := iActualProgramData.PullDownEnable;
  end
  else
  begin
    Result := iProgramProgramData.PullDownEnable;
  end;
end;

procedure TUSBPanel.fSetPullDownEnable( NewVal : boolean );
begin
  iProgramProgramData.PullDownEnable := NewVal;
end;
...
begin
  if iOpen then
  begin
    Result := iActualProgramData.SerNumEnable;
  end
  else
  begin
    Result := iProgramProgramData.SerNumEnable;
  end;
end;

procedure TUSBPanel.fSetSerNumEnable( NewVal : boolean );
begin
  iProgramProgramData.SerNumEnable := NewVal;
end;
...
begin
  if iOpen then
  begin
    Result := IntToHex( iActualProgramData.USBVersion, 4 );
  end
  else
  begin
    Result := IntToHex( iProgramProgramData.USBVersion, 4 );
  end;
end;

procedure TUSBPanel.fSetUSBVersion( NewVal : string );
begin
  iProgramProgramData.USBVersion := StrToInt( '$' + NewVal );
end;
  ...
  begin
    ...
    begin
      FT_Error_Check( 'attempting to set timeouts',
                      FT_SetTimeouts( iFT_Handle, ReadTimeout, WriteTimeout ));
    end;
  end;
end;
