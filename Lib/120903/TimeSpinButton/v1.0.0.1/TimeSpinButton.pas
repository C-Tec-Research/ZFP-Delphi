...
type
  ...
  public
    ...
    class function EncodeText( const NewVal : longint ) : string;
...
class function TTimeSpinButton.EncodeText( const NewVal : longint ) : string;
begin
 if NewVal >= 3600 then
 begin
   Result := Format( '%d:%2.2u:%2.2u', [ NewVal div 3600, (NewVal mod 3600) div 60, NewVal mod 60 ] );
 end
 else if NewVal >= 60 then
 begin
   Result := Format( '%d:%2.2u', [ NewVal div 60, NewVal mod 60 ] );
 end
 else
 begin
...
begin
  if IsInfinity then Text := '--'
  else Text := EncodeText( Value );
end;
