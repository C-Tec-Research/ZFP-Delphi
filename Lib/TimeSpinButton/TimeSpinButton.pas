...
type
  ...
  protected
    ...
    function PosIncrement : integer; // increment based on selpos
...
function TTimeSpinButton.PosIncrement: integer;
//var
//  iSelPos : integer;
begin
//  if SelLength = 0 then
    ...
    Result := Increment;
{
  ...
  begin
    iSelPos := SelStart + SelLength; // last position of selection determines increment
    iSelPos := Length( Text ) - iSelPos;
    case iSelPos of
      0: // secs
      begin
        Result := 1;
...
var
  iSelStart : integer;
  iWasInfinity : boolean;
begin
  if Value <> NewVal then
  begin
    iSelStart := Length( Text ) - SelStart - SelLength;
    ...
    begin
      iWasInfinity := IsInfinity;
      fHour := 0;
      fMin := 0;
      Seconds := NewVal;
      if not iWasInfinity then
      begin
        //Keep selection position
        SelStart := Length( Text ) - iSelStart;
      end;
    end;
  end;
end;
