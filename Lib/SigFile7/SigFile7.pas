...
type
  ...
  TSigFile7ParseResult = ( prSyntaxError, prValue, prBegin, prEnd,
                           prEmptyString, prComment );
  ...
  private
    ...
    function GetStructureChild(
      const pOfObject: TSigFile7BaseProperty): TSigFile7BaseData;
    //function GetSaveActiveItem: boolean;
  protected
  public
    ...
    property StructureChild[ const pOfObject : TSigFile7BaseProperty ] : TSigFile7BaseData
             read GetStructureChild;
    ...
    class function Matches( const pString, pTest : string ) : boolean;
    ...
    property Updating : boolean
             read GetUpdating
             write SetUpdating;
    property OnChangeText : TOnTextChange
             read fOnChangeText
             write fOnChangeText;
    const
      cEND        = 'END';
...
//  TSigFile7BaseProperty = class(TComponent)
  TSigFile7BaseProperty = class(TFMXObject)
  private
    fText: string;
...
//    function CreateCurrentItem : TSigFile7BaseData;
  ...
  protected
    ...
    procedure DoBeginUpdate; virtual;//override;
    procedure DoEndUpdate; virtual;//override;
  ...
  public
    ...
    function ChildWithName( const pWithName : string ) : TSigFile7BaseProperty;
    ...
    procedure BuildStructure( pData : TSigFile7BaseData ); // creates inactive children using our structure
    procedure MakeStructureInactive;
  ...
  published
    ...
    property OnLoadComplete : TNotifyEvent
             read fOnLoadComplete
             write fOnLoadComplete;
    {property Editors : TSigFile7TextComponentList
             read fEditorList;}
  end;
  ...
  TSigFile7DateTimeProperty = class( TSigFile7BaseProperty )
  private
  ...
  private
    ...
    function GetItemData(const i: integer): TSigFile7BaseData;
  protected
  ...
  public
    ...
    property ItemData[ const i : integer ] : TSigFile7BaseData
             read GetItemData;
...
begin
  ...
  begin
    if not (csDesigning in pChild.ComponentState) then
    begin
...
function TSigFile7BaseProperty.ChildWithName(
  const pWithName: string): TSigFile7BaseProperty;
var
  i : integer;
begin
  ...
  begin
    ...
    begin
      ...
      if SameText( Children[i].Name, pWithName ) then
      begin
        if Children[ i ] is TSigFile7BaseProperty then
        begin
          Result := Children[ i ] as TSigFile7BaseProperty;
          exit;
        end;
      end;
    end;
...
procedure TSigFile7BaseProperty.AfterConstruction;
begin
  inherited;
  if not (csDesigning in ComponentState) then
  begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        {iData := }TSigFile7BaseData.Create( pData, Children[ i ] as TSigFile7BaseProperty, FALSE );
        //(Children[ i ] as TSigFile7BaseProperty).BuildStructure( iData );
      end;
    end;
  end;
end;
...
begin
  ...
  begin
    if CanCreateCurrentData then   // this is always false in designing state
    begin
...
procedure TSigFile7BaseProperty.MakeStructureInactive;
var
  i: Integer;
  //iData : TSigFile7BaseData;
begin
  ...
  begin
    ...
    begin
      ...
      if Children[ i ] is TSigFile7BaseProperty then
      begin
        (Children[ i ] as TSigFile7BaseProperty).MakeStructureInactive;
      end;
    end;
  end;
end;
...
begin
  ...
  fActiveItem := -1;
  fIsItem := pIsItem;
  if not( csDesigning in pStructureOwner.ComponentState) then
  begin
    //fEditorList := TSigFile7TextComponentList.Create( FALSE );
    ...
    begin
      ...
      begin
        ...
        pOwner.StructureOwner.AddItemText( self, fText ); // this also updates editors
        {
...
begin
  ...
  // We use the local fCurrentData because we do not want current data to be auto
...
var
  i: Integer;
  iOwnerChild : TSigFile7BaseProperty;
begin
  for i := 0 to Children.Count - 1 do
  begin
    Result := Children[ i ];
  ...
  // else see if we can create it using our structure owners children.
  iOwnerChild := fStructureOwner.ChildWithName( pWithName );
  if assigned( iOwnerChild ) then
  begin
    if assigned( iOwnerChild.SigParent ) then
    begin
      if iOwnerChild <> iOwnerChild.SigParent.Item then
      begin
        // cannot create a list item like this
        Result := TSigFile7BaseData.Create( self, iOwnerChild, FALSE );
        exit;
      end;
    end;
  end;
...
function TSigFile7BaseData.GetStructureChild(
  const pOfObject: TSigFile7BaseProperty): TSigFile7BaseData;
begin
  Result := Child[ pOfObject.Name ];
end;
...
function TSigFile7BaseData.GetValueAsDateTime: TDateTime;
begin
  Result := StrToDateTimeDef( Text, Now());
end;
...
Retry:
    ...
    case iParseResult of
      ...
      begin
        ...
        begin
          ...
          Result := elError;
          pErrors.AddObject( cVALUE + ' without name - object ignored', TSigFile7ErrorObject.Create( Result, pFromLine, self ));
          inc( pFromLine );
          goto Retry;
        end
        else
      ...
      begin
        ...
        begin
          // we can only have nameless children in arrays
          Result := elError;
          pErrors.AddObject( cBEGIN + ' without name - object ignored', TSigFile7ErrorObject.Create( Result, pFromLine, self ));
          inc( pFromLine );
    ...
    if assigned( StructureOwner.OnLoadComplete ) then
    begin
      StructureOwner.OnLoadComplete( self );
    end;
  end;
end;

class function TSigFile7BaseData.Matches(const pString, pTest: string): boolean;
begin
  Result := SameText( Copy( pString, 1, Length( pTest )), pTest );
end;
...
begin
  ...
  begin
    fStructureOwner.MakeStructureInactive;
    // first nil any structure owner children current data
    // in case our children are not created yet
    if Value then
    begin
...
begin
  ...
  begin
    Updating := TRUE;
    try
      if assigned( fStructureOwner ) then
      begin
        if not (csDesigning in fStructureOwner.ComponentState) then
        begin
          if fText <> Value then
          begin
            IsDirty := TRUE;
          ...
          if assigned( fStructureOwner.OnChangeText ) then
          begin
            fStructureOwner.OnChangeText( self, Value );
          end;
          {
            Scoping Note. OnChangeText is a function that really should be ours
            but because of the way the IDE works it is simpler to store it in
            the structure owner. Despite the similarity in names the scopes of
            fStructureOwner.OnChangeText and fStructureOwner.ChangeText are quite
            different. fStructureOwner.OnChangeText is called whenever data is changed
            even if it is not current data. fStructureOwner.OnChangeText exits without
            action if we are not its current data.
          }
          if assigned( fOnChangeText ) then
          begin
            fOnChangeText( Self, Value );
          end;
        end;
      end;
    finally
...
procedure TSigFile7BaseData.SetValueAsDateTime(const Value: TDateTime);
begin
...
begin
  ...
  if SameText( Copy( iStr, 1, 3 ), cEND) then
  begin
  ...
  else if SameText( Copy( iStr, 1, 5 ), cBEGIN ) then
  begin
    Result := prBegin;
    iStr := Copy( iStr, 6 );
    iPos := Pos( '=', iStr );
  ...
  else if SameText( Copy( iStr, 1, 5 ), cVALUE ) then
  begin
    Result := prValue;
    iStr := Copy( iStr, 6 );
    iPos := Pos( '=', iStr );
  ...
  {
  else if SameText( Copy( iStr, 1, 5 ), cACTIVEITEM ) then
  begin
    iPos := Pos( '=', iStr );
    if iPos = 0 then
    begin
    ...
    begin
      pValue := Trim( Copy( iStr, iPos + 1 ));
      Val( pValue, iVal, iPos );
      if iPos = 0 then // <> 0 if not a valid number
      begin
        Result := prActiveItem;
      end;
      exit;
...
begin
  ...
  begin
    if iText = '' then
    begin
      pStrings.Add( Indent + cVALUE + ' ' + pName );
    end
    else
    begin
      pStrings.Add( Indent + cVALUE + ' ' + pName + ' = ' + iText);
    end;
  end
  else
  begin
    if iText = '' then
    begin
      pStrings.Add( Indent + cBEGIN + ' ' + pName );
    end
    else
    begin
      pStrings.Add( Indent + cBEGIN + ' ' + pName + ' = ' + iText);
    end;
    for i := 0 to Items.Count - 1 do
    begin
    ...
    begin
      ...
      {
      if SaveActiveItem then
      begin
        if (ActiveItem >= 0) and (ActiveItem < Children.Count) then
        begin
          pStrings.Add( Indent + '  ' + cACTIVEITEM + ' = ' + IntToStr( ActiveItem ) );
        end;
      end;
      }
    end;
    pStrings.Add( Indent + cEND + ' ' + pName);
  end;
end;
...
function TSigFile7List.GetItemData(const i: integer): TSigFile7BaseData;
begin
  if csDesigning in ComponentState then
  begin
    Result := nil;
  ...
  begin
    if assigned( Item.CurrentData) then
    begin
      Result := Item.CurrentData.Item[ i ];
    end
    else
    begin
      Result := nil;
...
procedure TSigFile7TextComponentList.SetText(const pNewValue: string);
var
  i: Integer;
begin
  BeginUpdate;
...
constructor TSigFile7DateTimeProperty.Create(AOwner: TComponent);
begin
  inherited;
  Value := Now();
end;

function TSigFile7DateTimeProperty.GetValue: TDateTime;
begin
  Result := StrToDateTimeDef( fText, Now());
end;

procedure TSigFile7DateTimeProperty.SetValue(const Value: TDateTime);
begin
