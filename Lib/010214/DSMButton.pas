    ...
    iOwnerDims : TButtonDims;
{
    ...
    procedure DoMouseMoveNormal( Shift: TShiftState; X, Y: Integer); virtual;
    procedure DoMouseUpNormal  ( Shift: TShiftState; X, Y: Integer); virtual;
    procedure DoMouseMoveEditing( Shift: TShiftState; X, Y: Integer); virtual;
    procedure DoMouseUpEditing(   Shift: TShiftState; X, Y: Integer); virtual;
}
    function Flat   : boolean;
//    procedure PaintRaisedButton( Canvas : TCanvas );
...
//    procedure fOnPaint( Canvas : TCanvas ); virtual;
//    procedure fOnPaint( MyBoss : TPaintbox ); virtual;
...
//    procedure OnPaint( MyBoss : TPaintBox; pOwnerDims : TButtonDims );
    procedure OnPaint( pOwnerDC : HDC; pOwnerDims : TButtonDims );
  end;
...
//    procedure fOnPaint( Canvas : TCanvas ); override;
//    procedure fOnPaint( MyBoss : TPaintbox ); override;
...
//  iColour := clRed;
  ...
  iUpdating := FALSE;
  iButtonStyle := 'General';
}
end;

destructor TDSMButton.Destroy;
begin
//  ReleaseDC( iOwnerWND, Canvas.Handle );
  Canvas.Free;
  inherited Destroy;
end;
...
//procedure TDSMButton.fOnPaint( Canvas : TCanvas );
//procedure TDSMButton.fOnPaint( MyBoss : TPaintbox );
procedure TDSMButton.fOnPaint;
begin
  // these are none functional buttons - hide unless in edit mode
  if Editing then
  begin
//    PaintRaisedButton( Canvas );
//    PaintRaisedButton( MyBoss.Canvas );
    PaintRaisedButton;
  end;
end;

//procedure TDSMButton.PaintRaisedButton( Canvas : TCanvas );
...
{
      Rectangle( iOwnerDims.Left, iOwnerDims.Top,
                 iOwnerDims.Left + iOwnerDims.Width,
      ...
      if not Flat then
      begin
//      Pen.Width := 1; // for sharp bevels
        for i := 1 to iBevelWidth do
        begin
          Pen.Color := clWhite;
{
          MoveTo( ClientRect.Left + i, ClientRect.Bottom - i - 1 );
          LineTo( ClientRect.Left + i, ClientRect.Top + i );
          LineTo( ClientRect.Right - i - 1, ClientRect.Top + i );
          Pen.Color := clGray;
          LineTo( ClientRect.Right - i - 1, ClientRect.Bottom - i - 1 );
          LineTo( ClientRect.Left + i, ClientRect.Bottom - i - 1 );
}
{
          MoveTo( iOwnerDims.Left + i, iOwnerDims.Top + iOwnerDims.Height - i - 1 );
          LineTo( iOwnerDims.Left + i, iOwnerDims.Top + i );
          LineTo( iOwnerDims.Left + iOwnerDims.Width - i - 1, i );
          Pen.Color := clGray;
          LineTo( iOwnerDims.Left + iOwnerDims.Width - i - 1, iOwnerDims.Top + iOwnerDims.Height - i - 1 );
          LineTo( iOwnerDims.Left + i, iOwnerDims.Top + iOwnerDims.Height - i - 1 );
}
...
//procedure TDSMButton.OnPaint( Canvas : TCanvas; pOwnerDims : TButtonDims );
//procedure TDSMButton.OnPaint( MyBoss : TPaintbox; pOwnerDims : TButtonDims );
procedure TDSMButton.OnPaint( pOwnerDC : HDC; pOwnerDims : TButtonDims );
begin
  iOwnerDims := pOwnerDims;
  Canvas.Handle := pOwnerDC;
//  fOnPaint( Canvas );
