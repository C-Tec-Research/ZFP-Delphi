...
{ DSM Changes:
  12/02/2016 Allow for 'T' prefix on Time conversion}
...
begin
   if (Length(s) > 0) and (s[1] = 'T') and (s[4]=':')  then          // DSM 12/02/2016
   begin
      Result := EncodeTime(ExtractNum(2,2), ExtractNum(5,2), ExtractNum(7,2), 0);
   end
   else if (Length(s) >= 8) and (s[3]=':') then
   begin
...
begin
   ...
   begin
      ...
      if (Length(s) > 10) and (s[11] = 'T') then
         Result := Result +  ISO8601StrToTime(Copy(s, 12));
   end
   else
